...
- 已确认必须恢复的形式：`Anon`（剧情对话、叙述和姓名字段不得写“匿名/你”）、`Tony`（不得写“托尼”）、`Tammy`（不得写“塔米”）、`Kevin`（不得写“凯文”）、`Judith`（不得写“朱迪丝/朱迪思”）、`Eddie`（不得写“埃迪”）。
...
| 英文 | 定译 | 范围 |
| --- | --- | --- |
| Master Blaster | 爆破大师；设备称爆破大师手柄 | 游戏、物品、对白和旧版任务提示同步；禁用“大师冲击”“主控爆破器” |
| falicum mushroom | 法利库姆蘑菇 | 配方与材料提示，不能省成“法利库蘑菇” |
| self-voicing / self voicing | 自动朗读 | 设置、辅助功能说明、启停提示；不用“自动语音”“自语音” |
| Father [self.clan] | [self.clan]神父 | 宗教头衔；亲属 father 仍按语境翻译 |
| PA | 广播 | 广播说话人名称，不是“物攻” |
| changing stall | 更衣隔间 | 不与整个更衣室混为一谈 |
| ward chart | 病房表 | 寻找住院人物的重复提示 |

任务提示中的角色 story 指“剧情”，不是角色正在口头“讲故事”。普通叙事中的 story 仍依语境处理。
...
| 原文 | 定译 | 使用边界 |
| --- | --- | --- |
| Aha! | 啊哈！ | 仅限发现、恍然的 Aha，全库 5 处 |
| Ah / Ahh / Aah | 啊（或依语境的哎呀、唉、哈） | 呻吟、呼唤、叹气式感叹；不得写成“啊哈” |
| aahh（被喘断） | 啊啊 | deb_shower 单处，保留拖长 |
| Hahaha… / m_laugh | 哈哈、哈哈哈…… | 笑声只重复“哈”，不夹“啊／啊哈” |
| *Gulp*（紧张吞咽） | *咽口水* | 全库主流；Debbie、Maria 的迟疑与紧张场景 |
...
| 原文及语境 | 采用译法 | 使用边界 |
| --- | --- | --- |
| early / midway / late in or through her story | 剧情前期／中段／后期 | 任务解锁进度，不能译为角色讲故事；保留角色变量与动作条件 |
| lounge（Debbie家中的任务） | 客厅 | 仅对住宅语境生效；教师休息室、酒吧休息区不能批量替换。共享UI字符串Lounge同时用于多个场所，不全局改为客厅 |
| camshow(s) | 成人直播 | 指定内容时可译为口交直播、手交直播；重复任务用“再开一场”，不用摄像头表演 |
| been packing（日记中的身体暗示） | 一直藏着这么个大家伙 | 保留原有暗示，不误译为打包装；不得用于所有 packing 语境 |
| a new vibe（日记购物语境） | 新振动棒 | vibe是vibrator的简称；其他表示气氛的vibe仍按上下文翻译 |

阁楼的 peephole 统一称“窥视孔”，不是门上的“猫眼”。以上改动以文件和完整原文登记于 sentence_patterns.json，避免跨语境替换。
