# plot 文件清单

7944源档案复核、日记覆盖、引擎验证和构建结果见`progress.md`最新记录。下方2026-08-03统计属于历史清单，不代表当前引擎的有效对白块数量；已按用户要求移除引擎确认的171个旧版orphan块。

更新时间：2026-08-03

用途与维护方式见[index.md](index.md)。

本清单由仓库当前文件自动统计，用于人工安排精修顺序；“推定分组”和“主要说话人”仅作为定位线索，完成翻译前仍须通读完整文件及前后剧情。

## 总览

- `tl/zh_hans/src/plot` 下共有 **260** 个 `.rpy` 文件。
- 共包含 **35,091** 个 `translate zh_hans` 块和 **353** 组 `old/new` 字符串。
- 数字连续序列无缺号：`ano01-16`、`bar01-06`、`deb01-27`、`dia01-02`、`jen01-28`、`jud01-02`、`mar01-02`、`mel01-06`、`tin01-02`、`tor01-06`、`viv01-05`。
- 未发现内容完全相同的重复文件。是否存在废弃场景不能仅凭文件名判断，需结合游戏原始脚本和实际跳转关系确认。

## 序章/教程

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `+prologue.rpy` | 122 | 20 | 0 | mono、more |
| `+tutor.rpy` | 167 | 25 | 0 | tutor、extend |

## 主线/Anon

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `ano01.rpy` | 3664 | 609 | 3 | anon、erik、annie、eve、kevin、ursula |
| `ano02.rpy` | 729 | 121 | 0 | harold、debbie、anon、yumi |
| `ano03.rpy` | 1521 | 253 | 0 | anon、yumi、debbie、dimitri、jenny、mono |
| `ano04.rpy` | 261 | 43 | 0 | tammy、debbie、anon |
| `ano05.rpy` | 693 | 115 | 0 | debbie、anon、mono、jenny |
| `ano06.rpy` | 1977 | 329 | 0 | anon、dimitri、tony、igor、debbie、jenny |
| `ano07.rpy` | 1497 | 249 | 2 | tony、anon、maria |
| `ano08.rpy` | 26 | 4 | 1 | anon |
| `ano09.rpy` | 2311 | 385 | 1 | tony、anon、maria、tina、mono |
| `ano10.rpy` | 2257 | 321 | 4 | maria、anon、tony、extend |
| `ano11.rpy` | 2279 | 325 | 2 | tony、maria、anon、dimitri、tina、igor |
| `ano12.rpy` | 32 | 5 | 1 | anon |
| `ano13.rpy` | 3791 | 541 | 2 | anon、tony、tina、maria、becca、missy |
| `ano14.rpy` | 476 | 79 | 1 | anon、maria、tony |
| `ano15.rpy` | 5165 | 754 | 3 | tony、anon、maria、mono |
| `ano16.rpy` | 315 | 52 | 0 | tony、anon、maria |

## Debbie线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `deb01.rpy` | 465 | 77 | 0 | debbie、anon |
| `deb02.rpy` | 447 | 72 | 4 | anon、debbie |
| `deb03.rpy` | 1331 | 220 | 3 | debbie、anon、jenny、mono、more |
| `deb04.rpy` | 1131 | 188 | 0 | anon、jenny、debbie、mono、more |
| `deb05.rpy` | 1155 | 190 | 4 | debbie、anon、mono |
| `deb06.rpy` | 758 | 126 | 0 | anon、debbie |
| `deb07.rpy` | 848 | 141 | 0 | debbie、anon、mono、more |
| `deb08.rpy` | 2282 | 376 | 6 | anon、debbie、kassy、mono |
| `deb09.rpy` | 199 | 33 | 0 | anon、debbie、ursula |
| `deb10.rpy` | 530 | 88 | 0 | anon、jenny、debbie |
| `deb11.rpy` | 531 | 88 | 0 | anon、jenny、debbie |
| `deb12.rpy` | 802 | 132 | 3 | anon、debbie、jenny |
| `deb13.rpy` | 2937 | 487 | 4 | anon、debbie、josie、jiang、mono |
| `deb14.rpy` | 1182 | 194 | 5 | anon、debbie |
| `deb15.rpy` | 253 | 41 | 2 | anon、debbie |
| `deb16.rpy` | 1689 | 281 | 0 | anon、debbie、hym、lsd、jenny、mono |
| `deb17.rpy` | 399 | 66 | 0 | debbie、anon |
| `deb18.rpy` | 1071 | 178 | 0 | debbie、anon、dimitri、jenny、mono、igor |
| `deb19.rpy` | 1863 | 304 | 10 | debbie、anon、mono |
| `deb20.rpy` | 927 | 154 | 0 | diane、debbie、anon |
| `deb21.rpy` | 1833 | 305 | 0 | debbie、anon、jenny、rue、neil、mono |
| `deb22.rpy` | 739 | 122 | 2 | debbie、anon、ursula |
| `deb23.rpy` | 3984 | 660 | 6 | anon、debbie、kassy、mono、TODO、more |
| `deb24.rpy` | 2055 | 342 | 0 | debbie、anon、diane、mono |
| `deb25.rpy` | 2571 | 428 | 0 | debbie、anon、lsd、hym、mono |
| `deb26.rpy` | 5526 | 853 | 7 | anon、debbie、diane、titomi、hana、kassy |
| `deb27.rpy` | 800 | 133 | 0 | debbie、anon |

## Debbie支线/场景

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `deb_baby.rpy` | 4612 | 758 | 16 | anon、debbie、jenny、diane、micoe、mono |
| `deb_island.rpy` | 925 | 153 | 2 | debbie、anon |
| `deb_kitchen.rpy` | 1233 | 205 | 1 | debbie、anon、jenny、mono |
| `deb_laundry.rpy` | 2003 | 333 | 2 | debbie、anon、mono |
| `deb_lobby.rpy` | 657 | 105 | 2 | debbie、anon |
| `deb_mall.rpy` | 3344 | 552 | 8 | debbie、anon、mono、more |
| `deb_pants.rpy` | 523 | 87 | 0 | debbie、anon |
| `deb_pool.rpy` | 339 | 56 | 0 | debbie、anon |
| `deb_shower.rpy` | 3163 | 526 | 2 | debbie、anon、mono |
| `deb_sink.rpy` | 1155 | 192 | 1 | debbie、anon |
| `deb_sleep.rpy` | 264 | 43 | 2 | anon、debbie |
| `deb_tv.rpy` | 4584 | 761 | 5 | debbie、anon、hym、mono、lsd |
| `deb_utility.rpy` | 80 | 13 | 0 | debbie、anon |
| `deb_visit.rpy` | 1777 | 295 | 2 | debbie、anon、mono |
| `debbie_attic.rpy` | 25 | 4 | 0 | mono、anon |
| `debbie_bed1.rpy` | 25 | 4 | 0 | anon |
| `debbie_bed2.rpy` | 43 | 7 | 0 | anon |
| `debbie_canvas.rpy` | 7 | 1 | 0 | anon |
| `debbie_drawer.rpy` | 21 | 3 | 0 | anon |
| `debbie_garage.rpy` | 49 | 8 | 0 | anon |
| `debbie_landing.rpy` | 25 | 4 | 0 | anon |
| `debbie_pants.rpy` | 43 | 7 | 0 | anon |
| `debbie_rug.rpy` | 405 | 67 | 0 | jenny、anon、mono、more |
| `debbie_tv.rpy` | 259 | 42 | 2 | anon、mono、debbie |
| `debbie.rpy` | 1277 | 207 | 9 | debbie、anon、TODO |

## Jenny线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `jen01.rpy` | 189 | 31 | 0 | anon、jenny |
| `jen02.rpy` | 351 | 58 | 0 | debbie、jenny、anon |
| `jen03.rpy` | 405 | 67 | 0 | anon、jenny、debbie、mono |
| `jen04.rpy` | 381 | 63 | 0 | anon、jenny、mans_voice |
| `jen05.rpy` | 931 | 129 | 7 | jenny、anon |
| `jen06.rpy` | 643 | 106 | 2 | anon、jenny |
| `jen07.rpy` | 1065 | 177 | 0 | anon、jenny、debbie |
| `jen08.rpy` | 1917 | 322 | 0 | anon、jenny、jane、ivy |
| `jen09.rpy` | 535 | 88 | 2 | anon、debbie、jenny、mono |
| `jen10.rpy` | 2331 | 388 | 0 | anon、jenny、grace、ivy |
| `jen11.rpy` | 288 | 46 | 0 | anon、jenny、debbie |
| `jen12.rpy` | 351 | 58 | 0 | anon、jenny |
| `jen13.rpy` | 451 | 75 | 0 | anon、jenny |
| `jen14.rpy` | 1248 | 192 | 0 | anon、jenny、debbie、cedric |
| `jen15.rpy` | 331 | 55 | 0 | jenny、anon |
| `jen16.rpy` | 1659 | 276 | 0 | jenny、anon、lily |
| `jen17.rpy` | 1767 | 293 | 0 | anon、erik、karl、pink、justin、jenny |
| `jen18.rpy` | 1083 | 180 | 0 | jenny、anon、mono |
| `jen19.rpy` | 343 | 57 | 0 | anon、jenny、debbie、mono |
| `jen20.rpy` | 439 | 73 | 0 | anon、jenny |
| `jen21.rpy` | 1035 | 172 | 0 | jenny、anon、mono |
| `jen22.rpy` | 357 | 59 | 0 | jenny、anon、debbie、mono |
| `jen23.rpy` | 1239 | 206 | 0 | jenny、anon |
| `jen24.rpy` | 1999 | 330 | 5 | jenny、anon、debbie、mono |
| `jen25.rpy` | 387 | 64 | 0 | anon、jenny、debbie、mono |
| `jen26.rpy` | 2697 | 449 | 0 | anon、jenny、zana、mono |
| `jen27.rpy` | 387 | 64 | 0 | anon、debbie、jenny |
| `jen28.rpy` | 969 | 161 | 1 | jenny、anon |

## Jenny支线/场景

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `jen_baby.rpy` | 6094 | 919 | 11 | jenny、anon、debbie、diane、micoe、extend |
| `jen_cam.rpy` | 1173 | 195 | 0 | jenny、anon、mono、extend |
| `jen_deal.rpy` | 43 | 7 | 0 | jenny、anon |
| `jen_finger.rpy` | 577 | 95 | 2 | jenny、anon |
| `jen_gfe.rpy` | 2197 | 365 | 2 | jenny、anon、mono |
| `jen_pool.rpy` | 939 | 156 | 0 | jenny、anon、mono |
| `jen_shower.rpy` | 1963 | 326 | 2 | anon、jenny |
| `jen_sleep.rpy` | 822 | 133 | 6 | jenny、anon |
| `jen_table.rpy` | 951 | 158 | 0 | jenny、anon、debbie、mono |
| `jen_tv.rpy` | 873 | 145 | 1 | jenny、anon |
| `jen_visit.rpy` | 235 | 38 | 2 | jenny、anon、mono |
| `jenny_laptop.rpy` | 903 | 150 | 0 | jenny、anon |
| `jenny.rpy` | 926 | 150 | 7 | jenny、anon |

## Diane线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `dia01.rpy` | 597 | 99 | 0 | diane、anon、mono、more |
| `dia02.rpy` | 837 | 139 | 0 | diane、anon、mono |

## Diane支线/场景

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `diane_lobby.rpy` | 19 | 3 | 0 | anon |
| `diane_plot.rpy` | 69 | 11 | 0 | anon、diane |
| `diane.rpy` | 65 | 9 | 3 | diane、anon |

## 酒吧线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `bar01.rpy` | 663 | 110 | 0 | barb、anon、ursula、mia、mono、more |
| `bar02.rpy` | 2025 | 337 | 0 | anon、barb、mia、eve、chad、mono |
| `bar03.rpy` | 1857 | 309 | 0 | anon、barb、mia、melody、kevin、jane |
| `bar04.rpy` | 3585 | 597 | 0 | anon、barb、judith、mia、ursula、mono |
| `bar05.rpy` | 2817 | 469 | 0 | anon、barb、mia、annie、ursula、lily |
| `bar06.rpy` | 1185 | 197 | 0 | barb、anon、iwanka |

## Maria线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `mar01.rpy` | 1095 | 167 | 0 | maria、anon |
| `mar02.rpy` | 171 | 28 | 0 | tony、anon |

## Maria支线/场景

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `mar_baby.rpy` | 2529 | 397 | 3 | anon、maria、tony、micoe |
| `mar_cook.rpy` | 272 | 67 | 2 | maria、anon |
| `mar_couch.rpy` | 3437 | 640 | 0 | maria、tony、anon、mono |
| `mar_dark.rpy` | 2590 | 370 | 1 | maria、tony、anon |
| `mar_door.rpy` | 560 | 140 | 0 | maria、anon |
| `mar_kitchen.rpy` | 231 | 38 | 0 | anon、maria |
| `mar_pantry.rpy` | 360 | 90 | 0 | maria、anon、extend |
| `maria_lounge.rpy` | 72 | 13 | 0 | anon、maria、TODO |
| `maria.rpy` | 385 | 60 | 7 | maria、anon |

## Melody线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `mel01.rpy` | 1887 | 314 | 0 | anon、melody、judith、erik、mono、ursula |
| `mel02.rpy` | 3235 | 475 | 0 | anon、eve、erik、melody、roxxy、kevin |
| `mel03.rpy` | 1039 | 167 | 0 | anon、melody、ursula、kevin、eve、annie |
| `mel04.rpy` | 724 | 181 | 0 | anon、melody、eve、tyrone、ursula、chad |
| `mel05.rpy` | 1108 | 277 | 0 | anon、erik、kevin、eve、mono、melody |
| `mel06.rpy` | 1344 | 336 | 0 | anon、melody、eve、kevin、mono、annie |

## Tina线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `tin01.rpy` | 181 | 30 | 0 | tina、anon、becca |
| `tin02.rpy` | 1030 | 167 | 0 | anon、tina、liu |

## Tina支线/场景

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `tin_baby.rpy` | 1762 | 338 | 4 | anon、tina、becca、TODO、micoe |
| `tin_cubicle.rpy` | 7 | 1 | 0 | anon |
| `tin_dusk.rpy` | 1001 | 167 | 0 | tina、anon |
| `tin_vault.rpy` | 1231 | 204 | 2 | tina、anon、liu、tim |
| `tina_lounge.rpy` | 212 | 31 | 3 | anon、tina、TODO |
| `tina.rpy` | 251 | 37 | 1 | tina、anon |

## Tori线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `tor01.rpy` | 2151 | 358 | 0 | anon、tori、konty、mia、mono、erik |
| `tor02.rpy` | 2829 | 471 | 0 | anon、tori、judith、erik、june、mono |
| `tor03.rpy` | 1599 | 266 | 0 | anon、tori、june、erik、mono |
| `tor04.rpy` | 627 | 104 | 0 | tori、ursula、anon、mono |
| `tor05.rpy` | 2727 | 450 | 7 | anon、tori、ursula、annie、mono、vee |
| `tor06.rpy` | 1053 | 175 | 1 | anon、tori、mono |

## Tori支线/场景

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `tor_office.rpy` | 375 | 62 | 0 | tori、anon |
| `tori.rpy` | 110 | 16 | 4 | tori、anon |

## Vivian线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `viv01.rpy` | 1322 | 219 | 3 | anon、viv、june、judith、jane、eve |
| `viv02.rpy` | 1664 | 276 | 3 | anon、viv、jane、ursula、mono、erik |
| `viv03.rpy` | 2138 | 354 | 5 | viv、anon、mia、roxxy、ursula、jane |
| `viv04.rpy` | 3771 | 626 | 5 | anon、roxxy、jenny、viv、bridget、debbie |
| `viv05.rpy` | 871 | 144 | 2 | viv、anon、mono、ursula、more |

## Vivian支线/场景

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `viv_office.rpy` | 85 | 14 | 0 | viv、anon |

## Ella线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `ell01.rpy` | 61 | 10 | 0 | anon、ella、mono |

## Josie线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `jos01.rpy` | 2359 | 392 | 2 | anon、josie、yoshi、yoo |

## Judith线

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `jud01.rpy` | 97 | 16 | 0 | judith、anon |
| `jud02.rpy` | 842 | 116 | 8 | anon、judith、camila、val |

## 独立角色/地点/物品场景

| 文件 | 行数 | 翻译块 | old/new | 主要说话人（推定） |
|---|---:|---:|---:|---|
| `ang.rpy` | 49 | 7 | 2 | anon、ang |
| `annie.rpy` | 88 | 14 | 2 | anon、annie |
| `anon_chair.rpy` | 31 | 5 | 0 | anon |
| `anon_pc.rpy` | 92 | 15 | 0 | anon、mono |
| `anon_phone.rpy` | 50 | 8 | 0 | anon |
| `apt_empty.rpy` | 25 | 4 | 0 | anon |
| `ari_stall.rpy` | 124 | 31 | 0 | anon、ariane |
| `bank_hall.rpy` | 15 | 2 | 0 | anon |
| `bank_lobby.rpy` | 43 | 7 | 0 | anon、more |
| `bank_vault.rpy` | 13 | 2 | 0 | anon |
| `bar_office.rpy` | 49 | 8 | 0 | anon、barb |
| `barb.rpy` | 287 | 40 | 12 | barb、anon |
| `bridget.rpy` | 155 | 25 | 2 | bridget、anon |
| `car_garage.rpy` | 13 | 2 | 0 | anon |
| `car_lounge.rpy` | 13 | 2 | 0 | anon |
| `car_shop.rpy` | 19 | 3 | 0 | anon |
| `cedric.rpy` | 473 | 77 | 3 | cedric、anon |
| `dexter.rpy` | 42 | 6 | 2 | dexter、anon |
| `ella.rpy` | 25 | 4 | 0 | anon、ella |
| `emm_stall.rpy` | 40 | 10 | 0 | anon、emma |
| `erik_bed.rpy` | 13 | 2 | 0 | anon |
| `erik_drawer.rpy` | 19 | 3 | 0 | anon |
| `erik.rpy` | 80 | 9 | 7 | anon、erik |
| `eve.rpy` | 275 | 44 | 3 | eve、anon |
| `gym_workout.rpy` | 13 | 2 | 0 | anon |
| `hana.rpy` | 377 | 59 | 8 | hana、anon |
| `helen_bed1_scope.rpy` | 49 | 8 | 0 | anon |
| `helen_bed2_scope.rpy` | 103 | 17 | 0 | anon |
| `helen_lobby.rpy` | 13 | 2 | 0 | anon |
| `ind_stall.rpy` | 68 | 17 | 0 | anon、indira |
| `ivy.rpy` | 103 | 16 | 2 | ivy、anon |
| `jane.rpy` | 47 | 4 | 6 | jane、anon |
| `jos_trade.rpy` | 833 | 133 | 9 | josie、anon |
| `josie.rpy` | 102 | 16 | 2 | josie、anon |
| `jud_stall.rpy` | 553 | 92 | 0 | judith、anon |
| `judith.rpy` | 124 | 19 | 3 | judith、anon |
| `june.rpy` | 72 | 11 | 2 | anon、june |
| `kassy.rpy` | 79 | 12 | 2 | kassy、anon |
| `kevin.rpy` | 152 | 25 | 0 | kevin、anon |
| `key_school.rpy` | 25 | 4 | 0 | anon |
| `konty.rpy` | 219 | 36 | 0 | konty、anon |
| `library_lobby.rpy` | 21 | 3 | 0 | anon |
| `library_shelf.rpy` | 239 | 38 | 3 | jane、anon |
| `library_study.rpy` | 135 | 22 | 0 | anon、jane |
| `lily.rpy` | 152 | 23 | 4 | lily、anon |
| `liu.rpy` | 102 | 16 | 2 | liu、anon |
| `mall_booth.rpy` | 25 | 4 | 0 | anon |
| `mall_hall1.rpy` | 18 | 2 | 2 | anon |
| `mel_office.rpy` | 397 | 98 | 2 | melody、anon |
| `melody.rpy` | 158 | 24 | 2 | melody、anon |
| `mia.rpy` | 62 | 10 | 1 | anon、mia |
| `micoe.rpy` | 37 | 5 | 2 | anon、micoe |
| `misc_lotion.rpy` | 13 | 2 | 0 | anon |
| `misc_tissue.rpy` | 13 | 2 | 0 | anon |
| `misc_toad.rpy` | 7 | 1 | 0 | anon |
| `misc_towel.rpy` | 13 | 2 | 0 | anon |
| `note_tori.rpy` | 19 | 3 | 0 | anon |
| `oli_stall.rpy` | 292 | 73 | 0 | olivia、anon |
| `photo_debbie_diane.rpy` | 13 | 2 | 0 | anon |
| `pie_stall.rpy` | 112 | 28 | 0 | anon、pietro |
| `pizza_boxes.rpy` | 273 | 45 | 0 | anon、maria、tony |
| `pizza_kitchen.rpy` | 13 | 2 | 0 | anon |
| `pizza_main.rpy` | 21 | 3 | 0 | anon |
| `pizza_shop.rpy` | 49 | 8 | 0 | anon |
| `roxxy.rpy` | 276 | 45 | 2 | roxxy、anon |
| `sam_stall.rpy` | 108 | 27 | 0 | anon、sammy |
| `school_boiler.rpy` | 7 | 1 | 0 | anon |
| `school_girls.rpy` | 19 | 3 | 0 | anon |
| `school_hall1.rpy` | 37 | 5 | 2 | anon |
| `school_locker.rpy` | 25 | 4 | 0 | anon |
| `school_office1.rpy` | 103 | 17 | 0 | ursula、anon |
| `school_office2.rpy` | 19 | 3 | 0 | anon |
| `school_pa.rpy` | 615 | 102 | 0 | pa、anon |
| `specs_judith.rpy` | 13 | 2 | 0 | anon |
| `sue_stall.rpy` | 40 | 10 | 0 | anon、sue |
| `sushi_shop.rpy` | 13 | 2 | 0 | anon |
| `tammy_bed1_scope.rpy` | 103 | 17 | 0 | anon |
| `tammy_bed2_scope.rpy` | 165 | 27 | 0 | anon |
| `tammy_lobby.rpy` | 25 | 4 | 0 | anon |
| `tammy_yard_scope.rpy` | 61 | 10 | 0 | anon |
| `tech_gamepad.rpy` | 13 | 2 | 0 | anon |
| `ten_stall.rpy` | 124 | 31 | 0 | anon、tenzin |
| `titomi.rpy` | 367 | 60 | 2 | titomi、anon |
| `ton_baby.rpy` | 884 | 152 | 1 | tony、anon |
| `tony.rpy` | 401 | 59 | 4 | tony、anon |
| `tool_drill.rpy` | 15 | 2 | 0 | anon |
| `tool_shovel.rpy` | 259 | 43 | 0 | anon、jenny |
| `tv.rpy` | 87 | 14 | 0 | anon |
| `tym_stall.rpy` | 28 | 7 | 0 | anon、tyme |
| `ursula.rpy` | 49 | 8 | 0 | anon、ursula |
| `vee.rpy` | 161 | 26 | 2 | vee、anon |
| `viv.rpy` | 158 | 24 | 4 | viv、anon |
| `yoo.rpy` | 281 | 45 | 3 | yoo、anon |
| `yoshi.rpy` | 84 | 13 | 2 | yoshi、anon |
| `zana.rpy` | 32 | 5 | 0 | zana、anon |



## 审校覆盖汇总

本节按线路汇总历次审校覆盖：原有按批次的日期标题已并入对应线路，日期只保留在下方摘要表中；具体条目、指纹与状态以 `manual_review.json` 和 `extracted_language_review.json` 为准。

| 日期 | 范围 | 记录数 |
| --- | --- | ---: |
| 2026-09-12 | 非露骨前后对白专项复核、全库筛查与第二遍语境复核 | 2 |
| 2026-09-13 | 连续审读、人工路径增量、任务与界面批次、提取文本拆分与命名 | 155 |

### 主线与序章

- **ano03–06中文连读**：新增740条：ano03 253、ano04 43、ano05 115、ano06 329。累计3626/37947条，其中双语连读2765条、中文优先连读861条。各本地场景、分支、回查原文范围已写入manual_review.json；未完成外部调度审校的状态不升级为全部完成。
- **ano07–09中文连读**：新增639条：ano07 250、ano08 4、ano09 385。累计4265/37947条，其中双语2765条、中文优先1500条。具体本地路径与疑点复核见manual_review.json，外部调度及小游戏仍待审。
- **ano10–11中文连读**：新增647条：ano10 322、ano11 325。累计4912/37947条，其中双语2765条、中文优先2147条。原文复核及本地分支已逐文件记录；完整外部触发与小游戏调度仍待审。
- **ano12–13中文连读**：新增546条：ano12 5、ano13 541。累计5458/37947条，其中双语2765条、中文优先2693条。所有本地路径与回查原文范围见manual_review.json；全文目标未完成。
- **ano14–15中文连读**：新增835条：ano14 79、ano15 756。累计6293/37947条，其中双语2765条、中文优先3528条。本地分支和原文疑点记录见manual_review.json，外部调度与跨文件重用尚未全部审完。
- **Maria家庭后续覆盖**：新增590条：mar01 167、mar02 28、mar_baby 395。累计7095/37947条，其中双语2765条、中文优先4330条。本地路径与原文回查详见manual_review.json；外部调度待审。
- **Tony育儿与Maria日常覆盖**：新增373条：ton_baby 152、mar_cook 68、mar_door 140、maria_lounge 13。累计7468/37947条，其中双语2765条、中文优先4703条。外部调度仍待审。
- **Tori主线收尾与重复入口**：tor06、tor_office、mini/serum新增238条，累计13899/37947条。
- **Jenny主线开端审校**：jen01–04新增219条人工覆盖，累计25475/37947条。

### Debbie 线

- **Tina约会覆盖**：新增371条，累计9238/37947条：双语2765条、中文优先6473条。本轮tin_dusk 166、tin_vault 205；本地路径已读，外部路径待审。
- **Debbie开场连续审校**：deb01–03新增374条人工覆盖，累计17347/37947条。
- **Debbie家务剧情连续审校**：deb04–05新增381条人工覆盖，累计17728/37947条。
- **Debbie按摩后续审校**：deb06新增126条人工覆盖，累计17854/37947条。
- **电影与购物连续审校**：deb07–08新增522条人工覆盖，累计18376/37947条。
- **重复购物分支审校**：deb_mall新增559条人工覆盖，累计18935/37947条。
- **梦境与醒后对白审校**：deb09–11新增209条人工覆盖，累计19144/37947条。
- **接吻教学与修车审校**：deb12–13新增624条人工覆盖，累计19768/37947条。
- **卧室与浴室后续审校**：deb14–15新增240条人工覆盖，累计20008/37947条。
- **电视与夜访连续审校**：deb16–17新增347条人工覆盖，累计20355/37947条。
- **入侵与陪睡连续审校**：deb18–19新增491条人工覆盖，累计20846/37947条。
- **密谈与烘焙节目审校**：deb20–21新增459条人工覆盖，累计21305/37947条。
- **和解购物连续审校**：deb22–23新增788条人工覆盖，累计22093/37947条。
- **泳池与浴袍分支审校**：deb24新增342条人工覆盖，累计22435/37947条。
- **夜间探索与回忆审校**：deb25新增428条人工覆盖，累计22863/37947条。
- **正式约会连续审校**：deb26新增858条人工覆盖，累计23721/37947条。
- **泳池场景连续审校**：deb27、deb_pool新增189条人工覆盖，累计23910/37947条。
- **家中入口连续审校**：debbie及六个家中场景提示文件新增245条人工覆盖，累计24155/37947条。
- **家中物件连续审校**：家中四个物件文件新增118条人工覆盖，累计24273/37947条。
- **电视事件连续审校**：deb_tv新增765条人工覆盖，累计25038/37947条。
- **望远镜后续审校**：jen23新增206条人工覆盖，累计28300/37947条。
- **泳池餐桌连续审校**：jen_pool、jen_table新增314条人工覆盖，累计30506/37947条。
- **夜间淋浴连续审校**：jen_sleep、jen_visit、jen_shower新增504条人工覆盖，累计31010/37947条。
- **家务场景连续审校**：deb_lobby、deb_laundry新增439条人工覆盖，累计31599/37947条。
- **厨房洗手台连续审校**：deb_kitchen、deb_sink新增397条人工覆盖，累计31996/37947条。
- **洗衣房入口与卧室连续审校**：deb_utility、deb_pants新增100条人工覆盖，累计32096/37947条。
- **Debbie夜间来访连续审校**：deb_visit新增296条人工覆盖，累计32392/37947条。
- **留宿岛台连续审校**：deb_sleep、deb_island新增198条人工覆盖，累计32590/37947条。
- **Debbie淋浴连续审校**：deb_shower新增527条人工覆盖，累计33117/37947条。
- **林间及影院连续审校**：ell01、ella、zana新增19条人工覆盖，累计33136/37947条。
- **设备提示与Roxxy连续审校**：anon_phone、anon_pc、roxxy新增69条人工覆盖，累计33205/37947条。
- **物品望远镜连续审校**：物品提示、公寓、电视及Helen家共9文件新增53条人工覆盖，累计33258/37947条。
- **隔间对白连续审校**：首批5个隔间文件新增162条人工覆盖，累计33420/37947条。
- **隔间第二批连续审校**：第二批4个隔间文件新增72条人工覆盖，累计33492/37947条。
- **Jenny怀孕第一段连续审校**：jen_baby仅源1—256新增89条人工覆盖，累计33581/37947条；本文件为部分审校，下一段从259继续。
- **Jenny怀孕第二段连续审校**：jen_baby覆盖推进至源620，新增116条，累计33697/37947条；仍为部分审校。
- **Jenny怀孕第三段连续审校**：jen_baby覆盖推进至源942，新增157条，累计33854/37947条；仍为部分审校。
- **Jenny怀孕第四段连续审校**：jen_baby覆盖推进至源1310，新增139条，累计33993/37947条；仍为部分审校。
- **Jenny怀孕第五段连续审校**：jen_baby覆盖推进至源1778，新增219条，累计34212/37947条；仍为部分审校。
- **Jenny怀孕末段连续审校**：jen_baby全文本地分支连读完成，新增209条，累计34421/37947条；外部依赖复核仍未完成。
- **Debbie怀孕第一段连续审校**：deb_baby部分覆盖至源844，新增317条，累计34738/37947条。
- **Debbie怀孕第二段连续审校**：deb_baby覆盖推进至源1350，新增175条，累计34913/37947条，仍为部分审校。
- **Debbie怀孕末段连续审校**：deb_baby全文本地分支覆盖，新增281条，累计35194/37947条，外部衔接复核未完成。

### Jenny 线

- **购车交易分支**：jos_trade/josie新增158条，累计16960/37947条。
- **Jenny电视连续审校**：jen_tv、jen20新增218条人工覆盖，累计25256/37947条。
- **Jenny照片连续审校**：jen05–06新增241条人工覆盖，累计25716/37947条。
- **Jenny交易连续审校**：jen07–08新增492条人工覆盖，累计26208/37947条。
- **Jenny同行购物审校**：jen09–10新增477条人工覆盖，累计26685/37947条。
- **Jenny电脑线索审校**：jen11–12新增89条人工覆盖，累计26774/37947条。
- **Jenny委托回报审校**：jen13–14新增260条人工覆盖，累计27034/37947条。
- **Jenny邀约连续审校**：jen15–16新增331条人工覆盖，累计27365/37947条。
- **面具任务连续审校**：jen17新增261条人工覆盖，累计27626/37947条。
- **首次直播连续审校**：jen18–19新增237条人工覆盖，累计27863/37947条。
- **直播后索款连续审校**：jen21–22新增231条人工覆盖，累计28094/37947条。
- **制服场景连续审校**：jen24新增334条人工覆盖，累计28634/37947条。
- **影院场景连续审校**：jen25、jen26新增513条人工覆盖，累计29147/37947条。
- **关系对白连续审校**：jen27、jen28新增225条人工覆盖，累计29372/37947条。
- **日常及女友体验连续审校**：jenny、jen_deal、jen_gfe新增529条人工覆盖，累计29901/37947条。
- **直播与前戏连续审校**：jen_cam、jen_finger新增291条人工覆盖，累计30192/37947条。
- **电脑视频连续审校**：jenny_laptop新增150条人工覆盖，累计31160/37947条。
- **电脑界面及邮件连续审校**：mini/pc新增77条人工覆盖，累计35271/37947条。gui/tel已初读但页面衔接未完成，本轮不计覆盖。

### Maria 与 Tony

- **披萨店日常入口覆盖**：新增212条：ano16 52、tony 59、maria 56、pizza_boxes 45。累计6505/37947条，其中双语2765条、中文优先3740条。具体本地路径和菜单回查记录见manual_review.json。
- **Maria房间场景覆盖**：新增764条：mar_kitchen 38、mar_pantry 90、mar_couch 636。累计8232/37947条，其中双语2765条、中文优先5467条。外部调度与共享镜头仍待审。
- **mar_dark覆盖**：新增369条，累计8601/37947条：双语2765条、中文优先5836条。ano15跨文件收尾复核不重复计数。mar_dark保持local_paths_reviewed，未宣称外部调度全审完成。
- **披萨店日常补读**：披萨店日常新增13条人工覆盖，累计16973/37947条。

### Tina 线

- **Tina入口覆盖**：新增266条：tin01 30、tin02 165、tin_cubicle 1、tina_lounge 33、tina 37。累计8867/37947条，其中双语2765条、中文优先6102条；本地路径已读，外部路径待审。
- **Tina孕产覆盖**：新增tin_baby 322条，累计9560/37947条：双语2765条、中文优先6795条。按本地分支记录，不等于外部任务链全部完成。

### Tori 线

- **Tori覆盖**：新增377条：tori 19、tor01 358。累计12328/37947条，其中双语2765、中文优先9563。
- **Tori镜片任务**：tor02新增471条，累计12835/37947条；本地中文连读不等于外部流程完成。
- **Tori腰带任务**：tor03新增266条，累计13101/37947条。
- **腰带故障后续**：tor04新增104条，累计13205/37947条。
- **Tori血清任务**：tor05新增456条，累计13661/37947条。

### Viv 线

- **viv02覆盖**：新增278条，累计9994/37947条，其中双语2765条、中文优先7229条。本地分支已读，外部调度待审。
- **viv01覆盖**：新增221条，累计10215/37947条，其中双语2765条、中文优先7450条。逐项记录本地分支，未宣称全量完成。
- **viv03覆盖**：新增358条，累计10573/37947条，其中双语2765条、中文优先7808条。保留外部路径待审标记。
- **viv04覆盖**：新增630条，累计11203/37947条，其中双语2765条、中文优先8438条。本地路径已读，外部触发仍待核实。
- **Viv考试覆盖**：新增187条：viv05 145、viv_office 14、viv 27、mini/french 1。累计11390/37947条，其中双语2765条、中文优先8625条。已读记录保留待处理问题与外部路径。

### Judith 线

- **Judith覆盖**：新增251条：judith 21、jud01 16、jud02 122、jud_stall 92。累计11715/37947条，其中双语2765条、中文优先8950条。

### Melody 与演出线

- **Melody长笛任务**：mel01原局部覆盖扩为全文，净增311条，累计14310/37947条。
- **演出招募与卡拉OK**：mel02新增473条，累计14783/37947条。
- **礼堂破坏事件**：mel03新增148条，累计14931/37947条。
- **礼堂清理与庆祝**：mel04新增181条，累计15112/37947条。
- **演出前夜行动**：mel05新增277条，累计15389/37947条。
- **才艺演出与收尾**：mel06新增336条，累计15725/37947条。
- **Melody日常与重复入口**：melody/mel_office新增124条，累计15849/37947条。

### Diane 与园艺

- **Diane开工任务**：dia01/tool_shovel新增142条，累计16013/37947条。
- **Diane搬运任务**：dia02新增139条，累计16152/37947条。
- **Diane日常及园艺界面**：Diane日常及园艺界面新增26条，累计16178/37947条。

### Barbara 美术线

- **美术后续人工路径增量**：新增569条，累计1416/37947条：bar05由157增至469条（本地全文），bar06全197条，bar_office全8条，barb全51条，mini/paint唯一提示1条。bar05_art2调色重试、bar06首次/重复与办公室位置分支、barb邀请接受/拒绝均已连读。
  已核对部分匹配版逻辑：取布及服装流转、预约日期与等待、画像后14天等待结果、获奖及首次完成后进入重复事件。完整事件装饰器、共享菜单与小游戏底层回调仍待审，相关文件保持local_paths_reviewed；数量不代表已验证所有运行路径。
- **美术前半段人工路径增量**：新增756条，累计2172/37947条：bar01全110条、bar02全337条、bar03全309条。对照源脚本连读全部本地标签、条件、被打断对白、场外发言和共用片段；关键物品与返回值逻辑另有核对记录。
  bar02的背包不同持有状态、Chad交换/找Eve索回两路线，bar03的三类杂志不同收集进度及Melody答题成功/失败均已记录。完整事件装饰器、共享角色菜单和bar04未读段继续待审，保持local_paths_reviewed，不把文件文本读完写成所有运行路径完成。

### 车行、银行与商店

- **银行与车行覆盖**：新增78条：liu 17、yoo 47、bank_lobby 7、bank_hall 2、bank_vault 2、car_shop 3。累计9638/37947条，其中双语2765、中文优先6873。
- **图书馆覆盖**：新增74条：jane 9、library_lobby 3、library_shelf 40、library_study 22。累计11464/37947条，其中双语2765条、中文优先8699条。
- **商店与借阅入口**：vee与mech/shop新增44条，累计13943/37947条。
- **商店角色接待**：lily/kassy/ivy新增56条，累计13999/37947条。
- **Josie车行开场**：jos01新增393条，累计16802/37947条。

### 学校与公共区域

- **学校同学入口覆盖**：新增42条：mia 10、kevin 25、dexter 7。累计11815/37947条，其中双语2765条、中文优先9050条。
- **校园区域覆盖**：新增136条：school_pa 102、school_hall1 6、school_locker 4、school_office1 17、school_office2 3、school_boiler 1、school_girls 3。累计11951/37947条，其中双语2765条、中文优先9186条。
- **学校任务道具补读**：六个道具提示与校长入口新增22条，累计15871/37947条。
- **健身入口与提示**：健身角色与界面新增45条，累计16223/37947条。
- **教练与餐厅接待**：Bridget/Hana/Titomi新增153条，累计16376/37947条。
- **公共地点接待补读**：公共地点接待和区域提示新增33条，累计16409/37947条。

### 界面、道具与提取文本

- **手机与背包界面审校**：gui/tel、gui/inv、gui/hud新增27条覆盖，累计35298/37947条。
- **任务提示首段审校**：step源1—142新增66条，累计35364/37947条。
- **任务提示第二段审校**：step覆盖推进至源214，累计35388/37947条。
- **任务提示第三段审校**：step覆盖至源274，累计35409/37947条。
- **任务提示第四段审校**：step覆盖至源344，累计35443/37947条。
- **任务提示第五段审校**：step覆盖至源407，累计35472/37947条。
- **任务提示第六段审校**：step覆盖至源431，累计35483/37947条。
- **任务提示第七段审校**：step覆盖至源477，累计35503/37947条。
- **任务提示第八段审校**：step覆盖至源525，累计35525/37947条。
- **任务提示第九段审校**：step覆盖至源588，累计35550/37947条。
- **任务提示第十段审校**：step覆盖至源639，累计35567/37947条。
- **任务提示第十一段审校**：step覆盖至源669，累计35583/37947条。
- **等待日期漏译补齐**：lang.wait五种返回值的补齐与覆盖变化见[progress.md](progress.md)对应批次；本清单只保留文件级统计。
- **任务提示第十二段审校**：step覆盖至源741，累计35614/37952条。
- **任务提示第十三段审校**：step覆盖至源794，累计35627/37952条。
- **任务提示第十四段审校**：step覆盖至源855，累计35650/37952条。
- **任务提示第十五段审校**：step覆盖至源899，累计35669/37952条。
- **任务提示第十六段审校**：step覆盖至源947，累计35688/37952条。
- **任务提示第十七段审校**：step覆盖至源988，累计35707/37951条。
- **任务提示第十八段审校**：step覆盖至源1018，累计35722/37951条。
- **任务提示第十九段审校**：step覆盖至源1054，累计35737/37951条。
- **任务提示第二十段审校**：step覆盖至源1099，累计35749/37951条。
- **任务提示第二十一段审校**：step覆盖至源1133，累计35764/37951条。
- **任务提示第二十二段审校**：step覆盖至源1178，累计35774/37951条。
- **菜单功能批量审校**：src/menu批次的处理与验收口径见[progress.md](progress.md)；本清单不重复批次记录。
- **功能批次审校与登记**：菜单12文件和小游戏交互21文件已登记本地人工覆盖；paint和pc沿用此前独立记录。bytecode、任务/道具元数据和共享界面仍在待审范围，未宣称全项目完成。
- **共享界面批次**：base_box/common_box.rpy和screens_box.rpy登记本地人工阅读覆盖。英文来源保留在修改前快照，关键引擎来源单列哈希依赖；不把旧game/screens.rpy注释当作当前源。
- **地点与场景名称批次**：地点两文件共185条全部连读登记；当前人工覆盖36535/37953。旧键逐项与7944的sets.name/alt核对，源行号更新及排序，影院不再堆放文件末尾。
- **角色资料批次**：res/meta/cast.rpy全部44条已登记本地人工覆盖，源文件为当前7944的cast.rpym；原文字段、变量及顺序保留。
- **道具分组审校**：prop.rpy完成中文前段至向日葵，源1—248；按物件名称和相邻说明联合审读。
- **道具分组续审**：prop.rpy前250条完成分组阅读；374起待续。
- **道具分组第三批**：prop.rpy前330条已有人工覆盖，源至503；颜料说明及后续物品仍待审。
- **道具合组收尾**：prop.rpy全部455条本地文本已读；品牌双关、卡牌图像与任务引用仍保留待核。
- **共用界面碎片合组**：12个零散文件的33条已有文本已合组阅读；voice调试分支字符串在其他翻译中的覆盖需另核。
- **本地文字阅读收尾**：bytecode_strings.rpy的631条本地文本均已阅读；36页日记有事件内分页记录。短信事件时序、图鉴实际条件及跨事件连接继续待核。
#### 提取文本按用途拆分（当前维护规则）

当前提取文本已由原`tl/zh_hans/bytecode_strings.rpy`迁至`tl/zh_hans/extracted/`，原聚合文件删除。历史章节中的旧路径只用于追溯。

| 文件 | 用途 | 条目 |
|---|---|---:|
| jenny_diary.rpy | 日记正文与旁注 | 294 |
| mesg.rpy | 手机短信 | 37 |
| lewd.rpy | 图鉴标题、变体和解锁提示 | 225 |
| anon_pc_jenny_laptop.rpy | 电脑、邮件与桌面应用 | 21 |
| cocktail_donut_garden_serum.rpy | 小游戏配料、材料等 | 12 |
| gui_wip_lewd_catalogue.rpy | 系统通知、开发中和目录状态 | 11 |
| pom_lang.rpy | 月相及相对日期 | 11 |
| ex_tv_television_menu_view_wind.rpy | 调试、技术字段及待定位的旧提取项 | 16 |
| game_seed.rpy | 旧存档名字迁移和姓名种子 | 4 |
631组old/new的键和值逐字保留；分组内保持原相对顺序。Ren’Py扫描语言目录下各rpy文件，不需要聚合入口。
- **提取文件按出现顺序整理**：9个extracted文件已按来源出现顺序重排，631组键值不变；日记附事件/页/行注释。manual_review中的items、diary_pages、message_events行号及引用这些文件的依赖哈希已同步迁移。此前拆分记录中“保持原相对顺序”仅描述当时状态，以本次排序规则为准。

### 全库筛查与覆盖口径

- **前后对白审校覆盖口径**：本次以21.0.0-wip.7944为准，对全部260个剧情翻译文件进行候选与入口定位；全库323个RPY另做结构和术语等静态审计。候选及补充阅读涉及132个剧情文件，包含通用人物交互、主线推进、初次／重复场景、忙碌拒绝、被打断、收尾、次晨及孕育后续。
  47个核心文件的2705条较长候选完成英中对照，其他69个文件的3426条候选完成译文审阅及重点英中复核；另补查74个文件中的5928条未入前两组的较长条目，补读jen20、june并检查短否定回答。上述组有文件交集，候选也可能含非目标剧情或露骨段落，数量不是场景完成率。保留正确译文，实际只修改44个文件243处译文。
  本轮完成的是已约定的非露骨前后对白专项系统复核，不是全部对白逐句重译，也不表示露骨段落已全面精修；未通过运行游戏逐场景验证。具体结果见progress.md，场景解释见storylines.md。
- **全库筛查与第二遍复核口径**：- 自动筛查323个文件、37947对原译文本。工具生成1347组重复候选、451组相似候选；人工处理20条专项标记、116条补充候选，并核对7组高差异重复句及2组同译相似句。候选可能重叠，不能相加为独立句数，也不代表逐一精读全部1798组。
  - 回看前轮244条非露骨修改记录（243处净改动）、62条连贯性记录及51条语义记录，按当前文件确认，包含已按用户反馈撤回的2条Tina修改；历史日志不是当前译文依据。
  - Jenny日记36页、296个片段按7944原始常量页序全读，修改后再次连读，重点复核分段、旁注指代及情绪转折。
  - 本轮净改动100处、16个翻译文件：bytecode_strings75处（74处日记、1处标题），prop5处，14个剧情文件合计20处。保留正确译文和角色有意使用的科学术语。
  - 这是全库筛查与疑点语境复核，不是37947条全部逐句重译或游戏内全分支验证。
#### 连续审读文件清单

完整读取下列文件的英中配对文本，共31个文件、2121对；包含菜单和空文本。疑点再查7944源码动作与分支，不能仅以相邻译文推断运行顺序。修改后复核关键句组及同源变体。此清单是本轮真实覆盖，不表示其余文件已完成逐句审读。

| 文件 | 原译对数 | 本轮改动 |
|---|---:|---:|
| barb.rpy | 51 | 5 |
| bar_office.rpy | 8 | 0 |
| diane.rpy | 11 | 2 |
| diane_lobby.rpy | 3 | 2 |
| diane_plot.rpy | 11 | 3 |
| erik.rpy | 15 | 3 |
| eve.rpy | 46 | 5 |
| judith.rpy | 21 | 1 |
| june.rpy | 12 | 0 |
| maria.rpy | 56 | 0 |
| melody.rpy | 25 | 0 |
| mia.rpy | 10 | 0 |
| roxxy.rpy | 46 | 0 |
| tin01.rpy | 30 | 5 |
| tina.rpy | 37 | 6 |
| tina_lounge.rpy | 33 | 6 |
| tin_cubicle.rpy | 1 | 0 |
| tony.rpy | 59 | 0 |
| tori.rpy | 19 | 3 |
| viv.rpy | 27 | 0 |
| viv_office.rpy | 14 | 0 |
| debbie.rpy | 215 | 25 |
| jenny.rpy | 156 | 2 |
| tor_office.rpy | 62 | 5 |
| bar01.rpy | 110 | 4 |
| dia01.rpy | 99 | 21 |
| jud_stall.rpy | 92 | 2 |
| mel_office.rpy | 99 | 0 |
| tin_dusk.rpy | 166 | 22 |
| viv01.rpy | 221 | 6 |
| jen_gfe.rpy | 366 | 10 |
- **人工路径台账增量**：本次新增804条记录，累计847/37947条。+tutor全24条；anon_chair全5条；ano01全611条（含3菜单）；bar04仅Annie模特邀请7条；bar05仅原脚本350–707范围157条。加上先前序章、Annie、Ang共43条。数量指逐项登记的已读文本，不是全部场景完成率。
  +tutor本地条件已核对；ano01等文件即使文本全读，外围事件注册与未核实的跨任务调用仍保留待审状态。bar04、bar05未读部分明确不计覆盖。后续以manual_review.json的条目、源范围和指纹为准。
- **审校覆盖增量**：bar04全文597条完成双语本地路径连读，较既有7条新增590条；mel01仅共享late制作提示新增3条，其余未计入。ano02全121条按中文优先方法连读，疑点原文回查范围另记。累计2886/37947条，其中双语连读2765条、中文优先连读121条；外部事件调度未完成的文件仍保留待审，不代表全项目审校完成。
- **Erik住处覆盖**：新增78条：erik 15、erik_bed 2、erik_drawer 3、tammy_lobby 4、tammy_yard_scope 10、tammy_bed2_scope 27、tammy_bed1_scope 17。累计9716/37947条，其中双语2765条、中文优先6951条。跨任务调度及共享镜头仍待审。
- **June/Eve覆盖**：新增58条：June 12、Eve 46。累计11773/37947条，其中双语2765条、中文优先9008条。
- **Konty覆盖**：新增36条，累计12364/37947条，其中双语2765、中文优先9599。
- **多任务链集中审校**：step源范围扩展至1—1508，人工覆盖累计36709/37953；本批按任务链联合阅读与统一修正。
- **剩余任务链批次**：step现有732条均有人工覆盖，覆盖至源1797。新增原文大小写变体，仓库总数37954。
