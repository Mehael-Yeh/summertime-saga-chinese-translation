...
| 日期 | 范围 | 结果 |
| --- | --- | --- |
| 2026-09-12 | 剩余语义修复、Lint 清理、编译包验证、发包后静态检查、mister 语境、英文残留、地名与间距专项、非露骨前后对白专项、全库筛查 | 结构与术语审计通过；逐条记录见下方批次记录 |
| 2026-09-13 | 连续审读、任务／界面／道具／角色资料批次、日记验收、提取文本拆分与命名、输入代码兼容、目录规范化 | 固定清单 65 条收敛；日记 294 条文本级通过；目录与一次性清单归档完成 |
...
- **330 对档双语复审（第六十五批）**：完成 jen24（334）逐条英中对照复审，1 处修正：Jenny 拉长喊出的 `FUUUUCK... ... YYOOOUUUU!!` 第二行原译“……你的！！”，与上一行连读会变成“操……你的”，改“……你！！”。其余核对无误：`cheerleading costume`→“啦啦队表演服”与 Jenny 坚持的“制服”之争、`head cheerleader`→“啦啦队长”、`Princess [jenny]`／斜体 `{i}Princess{/i}` 的后置“公主”、手铐惩罚与打赏音效、“我不是叫你别停”的内射两条分支、`Show's over, pervs`→“直播结束了，变态们”、`camslut`→“成人直播骚货”，以及菜单字符串 `Remain silent. {sub}`／`Say something. {dom}` 的标签保留都符合规则。台账 jen24 升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 1 条记录。
- **330 对档双语复审（第六十四批）**：完成 deb_laundry（334）逐条英中对照复审，2 处修正：文件内四处 `*Gulp*` 原有“*咕噜*”与“*咽口水*”两种写法，统一为全库主流的“*咽口水*”（全库 34 处）；开头 Debbie 被打断的 `Goodness, I-` 原译“天哪，你——”人称写反，改“天哪，我——”。其余核对无误：“load（一桶衣服／一泡精液）”的双关与三行拆分、洗衣机上的场景与震动档、玩家元梗（“同一个周目”）、`Naughty landlady`→“不安分的房东太太”、`That's my good boy`→“这才乖嘛”、`Tits` 被抓包的拌嘴、内射／射在身上两条分支与晚餐选牧羊人派都保留原味。台账 deb_laundry 升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 2 条记录。
- **330 对档双语复审（第六十三批）**：完成 ano06（329）逐条英中对照复审，3 处修正：Tony 听到 Anon 丧父后的 `Christ...` 原译“妈的”与全库 Christ／Jesus Christ 的主流（老天爷／我的天／天啊）不符，改“老天……”；`It was amazing!!` 与 `Yes, it is!!` 的译文各少一个感叹号，按“多重感叹号保留数量”的规则补足为“！！”。其余核对无误：Dimitri／Igor 的破碎英语与 rabbit 段子、`little bunny`→“小兔子”、数过十要用脚趾的嘲讽、洗手液争执、`From Russia with love` 用书名号、`Das vidania`→“达斯维达尼亚”、Russkies→“俄国佬”、债务／抵押房屋的整段崩溃戏与事后独白都保留原味；`?!?!` 按全库“？！”与“？！？！”并存的现状保留。台账 ano06 升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 3 条记录。
- **320 对档双语复审与 Slurp 收敛（第六十二批）**：完成 jen_shower（327）逐条英中对照复审，1 处修正：拉长的 `*Sluuuuurp*` 原译“*吸溜*”缺拉伸标记，改为“*吸溜——*”。沿拟声规则全库收敛 `*Slurp*` 的用字：deb_sink 的“*嘶溜~~~~~*”与 deb_tv 两处“*嘶溜嘶溜嘶溜*”改用统一起头“吸溜”（保留各自的波纹／重复表现音量）；style_guide 的 Slurp 规则同步改写为“统一用吸溜起头，拉长可作——／溜溜溜／波浪线，不用嘶溜”。jen_shower 其余核对无误：good boy／good doggy 的狗化玩笑按既有例外保留、`Princess [jenny]`／“求公主”后置称谓、`take it deep`→“再含深一点”、吞精与粉丝付费的争吵、内射／拔出两条分支都保留原味。台账 jen_shower 升级为 `bilingual_scene_read`，deb_sink／deb_tv 重算指纹，`extracted_language_review.json` 追加 4 条记录。
- **320 对档双语复审（第六十一批）**：完成 ano11（325）逐条英中对照复审，1 处修正：Dimitri 踹门时的 `*Crash*` 原译“哐当！！”缺星号并多出感叹号，改为“*哐当*”，与 deb25 的同源拟声一致；style_guide 的 `*Crash*` 规则同步细化为“保留星号，金属／器皿撞击可作哐当”。其余核对无误：领养争论与精子捐献者的荤段子、`The Plumber` 按专名保留英文（terminology 已有条目）、`little bunny`→“小兔子”、`babyface`→“小帅哥”、`champ`→“冠军”、四指Eddie 保留黑道称呼与英文名、Maria 掏枪护店与 `Get. The fuck. Out of my shop!` 的三行拆分、“死者为大”及老爸秘密／私生子对话、搬面粉两条分支都保留原味；俄语食物词按同文件既有做法贴中文（罗宋汤／俄式饺子／俄式馅饼）。台账 ano11 升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 1 条记录。
- **320 对档双语复审与重复句式收敛（第六十批）**：完成 tin_baby（322）逐条英中对照复审，10 处修正：`N-no` 的“不-不”改顿号；短信提示 `*Ding*` 补齐星号；`Can I get you anything?` 在文件内的 5 种写法（含 `translate strings` 菜单字符串）统一为“要我给你拿点什么吗？”；`Like I said, it's important to...` 的两种语序统一为“就像我说的，重要的是尽我所能确保宝宝出生时健康快乐。”；`Is there anything I can do?` 与 `How's everything going?` 各对齐菜单字符串；Becca 抱怨母亲那句补齐谓语，改“她连哺乳的建议都不让我提！”。其余核对无误：magical sperm 支线、避孕植入剂、单胎／双胎／三胞胎三条分支的逐句镜像、OB/GYN 与营养师／哺乳顾问的排场、“Mommy spared no expense”的育儿独白都保留原味。台账 tin_baby 升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 10 条记录。
- **320 对档双语复审（第五十九批）**：完成 ano10（322）逐条英中对照复审，1 处修正：Tony 开场的 `Boy, am I glad to see you.` 中 `Boy` 是感叹词，原译“好小子”误当称呼，改为“哎呀”。其余核对无误：champ→“冠军”、protégé→“徒弟”、mustache ride→“骑胡子”并保留 Anon 追问的笑点、`my boys can't swim`→“我的小蝌蚪游不动了”、“面糊过期／牛奶馊了／枪打空包弹”三重委婉到“精液里根本没有精子”的揭底、Luigi 与孤儿院／意大利黑手党的身世、夏日镇犯罪潮、以及“Oh. / My. / God.”三行拆分与 `[[{ctx=CHEAT}]` 作弊菜单字符串都按既有规则保留。台账 ano10 升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 1 条记录。
- **310 对档双语复审（第五十八批）**：完成 jen08（315）逐条英中对照复审，无需改动。本篇是 Pink（粉色诱惑）店内的买玩具线：`The Electro Clit`→“电击阴蒂棒”、`Electro Clit Light`→“轻量版”、Anon 记错成“电……什么蒂”／“电击拼贴”（Electoral College 笑点）、Ivy 与 Jane 的按摩交易、Jenny 的羞辱条件（“我是个可悲的小废柴，这辈子都只能当处男”）、只准看一分钟与最后“别在这里说”都保留原味。文件内未发现重复源句译法漂移、破折号结巴或半角标点；`fingering` 的“用手指肏”与项目露骨度一致。台账 jen08 升级为 `bilingual_scene_read`（本轮无译文改动，故不追加 `extracted_language_review.json` 记录）。
- **310 对档双语复审（第五十七批）**：完成 mel01（314）逐条英中对照复审，7 处修正：Ursula 挖苦后的 `Grrr...` 由“唔……”统一“哼……”；`[tyrone].` 的句末半角句号改全角；Judith 的五处结巴由破折号／省略号改为顿号（“我——我”“你——你好”“好——好了”“随——随便”“是……是的”）。其余核对无误：Melody 的口癖 sugar→“甜心”、长笛／口交双关（handle his flute／work miracles with this mouth／length and girth／breaking in new instruments）、Judith 把长笛坐坏的自嘲线、Raven Hill→“渡鸦山”、World of Orcette 的兽人村庄与群交梗、以及“找笛子→造笛子→音乐会”的三段任务链都保留原味；flute 在项目里有“长笛／笛子”两种自然说法，本轮不改。台账 mel01 升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 7 条记录。
- **310 对档双语复审与拟声收敛（第五十六批）**：完成 deb19（313）逐条英中对照复审，7 处修正：三处 `*Ahem*` 由“*清嗓子*”统一“*咳咳*”；三处 `*Sluuuuuuurp*` 由“*用力吸吮*”统一“*吸溜——*”；同一句 `Fffffaaaa...` 在文件内原有两种写法，统一“呼啊啊……”。沿拟声规则全库收敛 `*Gasp*`：把 deb13／deb_visit／jen_shower／tor04 的“*倒吸一口凉气*”、ari_stall 的“*倒抽一口气*”、jen_baby 的“*倒抽一口冷气*”、deb_shower 的“*吸气*”共 7 处统一为“*倒吸一口气*”；性爱语境的“*喘息*”与被呛后的“*大口喘气*”属语境分工保留，规则同步写入 style_guide。deb19 其余核对无误：助眠药与空床独白、邀请同床、`good boy`／`my wonderful boy` 照顾者亲称、口交与首次插入分支、“尊重我的底线”“真做了就回不到从前”的边界对话，以及 10 个菜单选项都保留原味。台账 deb19 升级为 `bilingual_scene_read`，其余 7 个文件重算指纹，`extracted_language_review.json` 追加 14 条记录。
- **290 对档双语复审与结巴／拟声收敛（第五十五批）**：完成 deb_visit（296）逐条英中对照复审，13 处修正：`my wonderful boy` 由“我的好男孩”改“我的好孩子”（与 deb19 同句定译及“不写母子式好男孩”的既有决定一致）；结巴统一用顿号（“哦——哦”／“好——好吧”／“对对对不起”→“哦、哦”／“好、好吧”／“对、对不起”）；`so very very wrong` 的“太、太太”改为“太、太”（斜体落在第二个“太”）；`Sluuuuuurp` 的“吸溜————”统一“吸溜——”；`Ahem` 统一“咳咳”；错字“恩啊啊”改“嗯啊啊”；`Mm-uah` 与全库 Muah 统一“啵”；`*Crash*` 统一“*砰*”；文件内重复的 `Whatsit?` 与“{i}*哈欠*{/i} 早——！”两处写法各自统一。顺带把 mel06 的 `Muah!`“木啊！”一并改为“啵！”。性爱过程中的“想念／不是错过”处理、孕妇激素驱动的夜访与拒绝分支均核对无误。台账 deb_visit 升级为 `bilingual_scene_read`，mel06 重算指纹，`extracted_language_review.json` 追加 14 条记录。
- **280 对档双语复审与截断句收敛（第五十四批）**：完成 deb16（281）逐条英中对照复审，1 处修正：截断式 `What the-` 由“这什么——”统一“搞什么——”。沿同一问题全库收敛截断式 `What the-` 的其余写法：deb24 的“什么鬼——”、mar_baby 与 tor02 的“什么——”共 3 处统一为“搞什么——”；jen_shower／jen_sleep 的“搞什——”属被硬切断的刻意写法，保留不动。deb16 其余核对无误：烘干机卡人的色情片剧情与 Debbie 逐句吐槽、`Language!`→“注意言辞！”、landlady 角色扮演称呼、油／打屁股／脱内裤的推进、以及事后“我们俩都很快乐”的关系确认都保留原味。台账 deb16 升级为 `bilingual_scene_read`，deb24／mar_baby／tor02 重算指纹，`extracted_language_review.json` 追加 4 条记录。
- **270 对档双语复审与西语／法语补漏（第五十三批）**：完成 viv02（278）逐条英中对照复审，4 处修正：开头 `Non` 由“不”改回“Non（不）”；Val 的两次西班牙语辱骂 `culo` 由“臭小子”还原为“culo（屁眼）”，与 jud02 的 `PUTA!（婊子！）` 处理一致；Viv 的 `Aww, tellement mignon...` 原被整句汉化，还原法语并补“（真可爱）”，补上“无重音符号的法语也要还原”这条判据的漏网个案。顺带统一书名：prop.rpy 的 `Chola's Tricks` 由“乔拉的妙招”改为“Chola的招数”，与 storylines.md 及 viv02 的《Chola的招数》一致（chola 是西语俚语而非人名）；同一道具说明的 skank ass hoes 由“贱人”改“骚货”，不净化原文。规则写入 terminology（tellement mignon 行、culo／PUTA／Chola 说明）。台账 viv02 升级为 `bilingual_scene_read`，prop.rpy 重算指纹，`extracted_language_review.json` 追加 6 条记录。
- **270 对档双语复审与省略号间距收敛（第五十二批）**：完成 mel05（277）逐条英中对照复审，13 处修正：Ursula 与 Annie 两人的代词由“他们”统一为“她们”（6 行、7 处，其中拔电话那句 `her phone` 按单数改“她”）；`Operation Gluteous Maximum` 由“屁股最大行动”改为“臀大肌行动”，恢复 gluteus maximus 的双关；`Operation Sticky Chicken` 由“黏糊糊烤鸡”改“黏糊鸡”，去掉原文没有的“烤”；`Yes! Thank you` 由“是的！”改“太好了！”（欢呼而非应答）；三处“…… ”多余空格顺手收敛。顺带全库收敛省略号写法：把剩余 25 处“…… ”（省略号后多一个空格，共涉及 jenny_diary、step、bar02、deb_visit、debbie_rug、jen_baby、mar_couch、mel02、mel06、tin_baby、tor02 等 11 个文件）改为“……”，规则写入 style_guide。台账 mel05 升级为 `bilingual_scene_read`，其余 11 个文件重算指纹，`extracted_language_review.json` 追加 38 条记录。
- **270 对档双语复审（第五十一批）**：完成 jen16（276）逐条英中对照复审，3 处修正：两处“小联盟棒球棒”比喻原写“跟根少年棒球棒”（“跟根”叠字笔误），统一改为“就像根少年棒球棒”；`act like a bitch` 由“这么刻薄”改“这么撒泼”，保住原文的斥骂力度。其余核对无误：camshow 议价与分成、盛夏买滑雪面罩的梗、`dick holster` 的粗俗说法、honey than vinegar 的中文化、`Not for free, you don't!` 的主客关系都成立；Jenny 叫 Debbie 的 Mom 出现“妈妈／妈”两种写法，terminology 允许按角色口吻处理，本轮不动。台账 jen16 升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 3 条记录。
- **260 对档双语复审（第五十批）**：完成 tor03（266）逐条英中对照复审，7 处修正：`dumber than a box of rocks` 由直译“比一箱石头还笨”改为中文说法“笨得跟块石头似的”；`Well shoot!` 由“操”降为“哎呀，糟了！”，恢复轻度感叹；`not an authentic-` 改“不是正品——”，与同文件 authentic→正品一致；结巴“对-对不起”改回顿号“对、对不起”；两处 Grr 的“呃／啧”统一为“哼”；被打断句“这不太-”的半角连字符改全角破折号“——”，并删掉“……”后的多余空格。规则补入 style_guide（结巴用顿号、打断与拖长用全角破折号）。faptic engine→触觉引擎、Master Blaster→爆破大师／《爆破大师》、Okitatron 与压电纤维等专名均与既有定译一致。台账 tor03 升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 7 条记录。
- **260 对档双语复审（第四十九批）**：完成 jen17（261）逐条英中对照复审，6 处修正：`What the-` 由“什么情况——”统一“搞什么——”；`joker` 的“逗比”改“活宝”；`34-0`／`34-0-1` 去掉中文与数字之间的空格；`cosplay community` 由“玩角色扮演的”改“cosplay圈”；Erik 的 `Muahahaah!` 由“哈哈哈”改“哇哈哈哈”，补回反派式大笑的得意语气。顺带把 `prop.rpy` 的 `A couple of cosplay outfits` 由“几件角色扮演服装”统一为“几件cosplay服装”，并把 terminology 的 cosplay 行扩写为项目级规则（cosplay 保留原文，只有 `LARPer` 才写“角色扮演”）。台账 jen17 升级为 `bilingual_scene_read`，prop.rpy 重算指纹，`extracted_language_review.json` 追加 7 条记录。
- **220 对档双语复审（第四十八批）**：完成 deb03（222）、ano07（250）共 2 个文件、472 对逐条英中对照复审，5 处修正：`deb03` 的 `fuelish`（fuel／foolish）双关原译“汽油可不是闹着玩的”丢掉笑点，改为“可别在这儿闹出什么‘油’漏”；`Embrace the chaos` 由“豁出去了”改“乱就乱吧”。`ano07` 的 `speak of the devil` 由“说人就到”改用中文成语“说曹操曹操到”；Tony 问 You got a bike? 的菜单选项由“是／否”改为与对白一致的“有／没有”。规则补入 style_guide（英语习语按中文既有成语落地）与 terminology（`Magnifico!`／`Bravo!` 按英语口语汉化，只有 Tony 的 `capisce／capiche` 保留意大利语加括注）。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 5 条记录。
- **200 对档双语复审与第三语言补漏（第四十七批）**：完成 debbie（215）、viv01（221）共 2 个文件、436 对逐条英中对照复审，5 处修正并补 1 处法语漏译：`debbie` 的 Grrrrr! 由“咕噜噜”（中文里是肚子响）改为与 jen_shower 同源拟声一致的“哼——！”；`Heh, when did you turn in to such a naughty boy?` 由“变得这么坏了”改“变得这么不老实了”。`viv01` 为 parlez-vous Français? 补上中文括注，并把被整句汉化的 `Ah, quel charmeur!`、`Ohh, tu me flattes...` 还原法语原文加释义。顺带用不含重音符号的第三语言清单重扫全库，补上 viv03 被汉化的 `Ça m'excite!`；新增规则“排查第三语言漏译不能只按重音符号筛选”，并把 `en Français`／`Madame`／`Voila` 等按英语借词处理的情形写进 terminology，避免反复返工。台账 debbie／viv01 升级为 `bilingual_scene_read`，viv03 重算指纹，`extracted_language_review.json` 追加 6 条记录。
- **200 对档双语复审与反问句收敛（第四十六批）**：完成 tin_vault（205）、jen23（206）共 2 个文件、411 对逐条英中对照复审，12 处修正：`tin_vault` 的 `Not yet.` 由“还没呢”改为对 Tina 的阻止“还不行”；`Comfy?` 的回答 `Not really.` 由误译“不太确定”改“不太舒服”；`Mwah` 由“么啊”统一“啵”；`shut up and squat on this dick` 由“蹲下来吃这根鸡巴”改“快坐上来”（贴合 p_climb 骑乘动作，避免读成口交）。`jen23` 的 `I have to give it to you` 由“不得不说”补全为“我不得不服你”；`Damnit` 由“妈的”统一“该死”；含混句 `Urfs ma uhnny!` 由不成立词的“这唔好笑”改为按语义还原的“这……不……好……笑！”；`Sheesh, you soaked me` 的“天，”补成“天啊，”。沿同一问题全库收敛：`Damn it, [jenny]...` 在 jen26 也统一“该死”；`What the X, [name]?!` 在 deb24／jen_sleep／jenny 的“搞什么啊”统一为全库主流的“搞什么鬼”，规则写入 style_guide。台账 tin_vault／jen23 升级为 `bilingual_scene_read`，jen26／deb24／jen_sleep／jenny 重算指纹，`extracted_language_review.json` 追加 12 条记录。
...
- **170 对档双语复审（第三十九批）**：完成 jen21（172）、tor06（175）共 2 个文件、347 对逐条英中对照复审，3 处修正：`jen21` 长串打赏音效里 13 处“*叮* {w=.03}”多出空格，统一为同文件其他行的“*叮*{w=.03}”；`tor06` 的 YEESSSSSS 由“是啊啊啊啊”改“对啊啊啊啊”（与同为 Tori 场面的 tor_office／tor04 一致），并删掉“啊——，”里多余的逗号。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 3 条记录。
- **150 对档双语复审（第三十八批）**：完成 tin_dusk（166）、mar01（167）共 2 个文件、333 对逐条英中对照复审，无需改动。`tin_dusk` 中用户确认过的 `Pound that pussy!`→“肏烂我的骚屄！”保持原样，doggy／big boy／`Double-oh-six-seven, license to drill` 等说法也都与原文一致；`mar01` 的 honey baked hams 火腿玩笑、`Finish inside me`／`Put another baby in me` 的直白求和与“老板娘”称呼都保留原味。台账 2 个文件升级为 `bilingual_scene_read`。
- **150 对档双语复审（第三十七批）**：完成 jen_table（158）、tin02（165）共 2 个文件、323 对逐条英中对照复审，3 处修正：`jen_table` 的被打断半句 `Could you not-` 不再补完为“你能别喊她吗”，改“你能不能别——”；同文件 What the fuck, [anon]?! 由“搞什么啊”统一到全库主流的“搞什么鬼”。`tin02` 中 Tina 的 Oh, you naughty boy 属调情，按 style_guide 由“坏小子”改“小坏蛋”（与同角色 tin01 一致）。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 3 条记录。
- **150 对档双语复审（第三十六批）**：完成 jen_pool（156）、jenny（156）共 2 个文件、312 对逐条英中对照复审，无需改动。`jen_pool` 的水下咕噜拟声、`big baby`→“胆小鬼”、CANNONBALL→“炸弹入水”与 Princess [jenny] 称呼都正确；`jenny` 的 GFE／现金／面具菜单、*Snort*→“哼”、`[saga.cast.anon!u]` 变量与各处打断句都保留原样。台账 2 个文件升级为 `bilingual_scene_read`。
- **150 对档双语复审（第三十五批）**：完成 ton_baby（152）、deb_island（154）共 2 个文件、306 对逐条英中对照复审，1 处修正：`deb_island` 里 Debbie 说“得快点”后 Anon 的 `Mm, I can do that.` 原译“嗯，这我知道。”把同意说成了知情，改“嗯，没问题。”。`ton_baby` 无需改动：Luigi 的几段黑帮段子（wingman／blood bank／ciabatta／Say hello to my little friend）与 he／she／they 性别分支都一一对应。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 1 条记录。
- **150 对档双语复审（第三十四批）**：完成 mel03（148）、jenny_laptop（150）共 2 个文件、298 对逐条英中对照复审，3 处修正：`mel03` 把 Eve 说的 Guys, come quick 从“兄弟们”改“大家”；brimstone and sulfur 原译“硫磺和硝石”改“硫磺和硫磺石”；删掉“夫人 {i}就是{/i}”标签前多余空格。`jenny_laptop` 无需改动：Pink频道账号、打赏 PING 节奏、sam9 互动与 YES 口气都保留原味。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 3 条记录。
- **140 对档双语复审与法语还原（第三十三批）**：完成 jen_tv（145）、viv05（145）共 2 个文件、290 对逐条英中对照复审；jen_tv 无需改动（Pink Channel、Princess [jenny] 语序、内射分支均正确）。viv05 修正 5 处：Ça me saoûle／S'il te plaît／Ah, ma chatte…／Ohh, elle est si grosse／Ohh, elle est toujours aussi grosse 原被整句汉化，按非英语规则还原法语加括注。同时全库扫描带重音符号的原文，把 viv01（3 处）、viv02（2 处）、viv03（4 处）、viv04（1 处）共 10 处 Très bien／Génial／Je suis désolée／Lequel 一并还原，其余 15 个候选（Crèche、Krüger、Rodríguez、protégé、Déjà vu、en Français 等）属地名、人名或英语借词，未改。台账 jen_tv／viv05 升级为 `bilingual_scene_read`，viv01–04 重算指纹，`extracted_language_review.json` 追加 15 条记录。
- **140 对档双语复审（第三十二批）**：完成 deb07（141）、jos_trade（141）共 2 个文件、282 对逐条英中对照复审，无需改动。`deb07` 的电影之夜确认 naughty 在电影语境用“少儿不宜”、`I'm solid` 的遮掩双关（挺得住）成立、Brazilian Bum Bum 品牌与分段独白都保留原味；`jos_trade` 的 Mini Vulva／Overcompensator／SL-700 Crotch Rocket 模型名、源文拼写错误（fourty）和菜单变量标签都按原文处理。台账 2 个文件升级为 `bilingual_scene_read`。
- **130 对档双语复审（第三十一批）**：完成 dia02（139）、mar_door（140）共 2 个文件、279 对逐条英中对照复审，1 处修正：`dia02` 搬手推车使劲时的 `Ghhh...` 原译“咳……”是咳嗽声，改“呃……”。`mar_door` 无需改动：*Knock* 已是统一的“*咚咚* *咚咚*”，Maria 的“腰实在搬不动”、`cawk`、`blue drinks` 与事后收尾都保留原味。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 1 条记录。
- **130 对档双语复审（第三十批）**：完成 deb27（133）、jen05（134）共 2 个文件、267 对逐条英中对照复审，4 处修正。`deb27`／`jen_baby` 的 `hear you` 原译“听见你”不合中文搭配，改“听见的”（全库 3 处“听见你”里 mar_door 的“听见你这么说”属正确用法，未动）；`jen05` 两处 `go back to Consum-R` 译成“回购乐百货”会被读成“回购”，分别改为“回购乐百货去上班”“回去，到购乐百货问问”。台账 deb27／jen05 升级为 `bilingual_scene_read`，jen_baby 重算指纹，`extracted_language_review.json` 追加 4 条记录。
- **120 对档双语复审（第二十九批）**：完成 jud02（122）、deb06（126）共 2 个文件、248 对逐条英中对照复审，1 处修正：`jud02` 中 Camila 的西班牙语骂人 `PUTA!` 原被直接译成“婊子”，按非英语保留原文加括注的规则还原为“PUTA!（婊子！）”。`deb06` 无需改动：Brazilian Bum Bum 品牌保留、三次 `*SNII…FFF*` 统一作“深深吸气”、That's my good boy 用家族定译“这才乖嘛”。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 1 条记录。
- **100+ 对档双语复审（第二十八批）**：完成 ano05（115）、ano02（121）共 2 个文件、236 对逐条英中对照复审。正文无需改动：ano05 的 You're such a good boy／That's my boy 已是家族收敛后的定译，ano02 的案情陈述、10-62、bedtime bandit、[saga.cast.liu] Wang 姓名顺序都保留原样。复审中发现哭泣语境的 {i}*Sniff*{/i} 有“抽泣／抽鼻子／吸鼻子”三种写法（9:5:3），本轮把 ano02、deb21、deb23、deb_baby 共 9 处统一为“抽泣”，规则记入拟声一致性。台账 ano05／ano02 升级为 `bilingual_scene_read`，另 3 个文件重算指纹，`extracted_language_review.json` 追加 9 条记录。
- **100 对档双语复审（第二十七批）**：完成 deb_lobby（105）、jen06（107）共 2 个文件、212 对逐条英中对照复审，1 处修正：`deb_lobby` 的 `[jenny] might hear you` 原译“可能会听见你”不合中文搭配，改“可能会被听见”。`jen06` 的羞辱、两分钟／三十秒讨价还价与 `Fine. {sub}`／`Screw you. {dom}` 标签都保留原样。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 1 条记录。
- **100 对档双语复审（第二十六批）**：完成 tor04（104 对）逐条英中对照复审，9 处修正：Tori 被遥控腰带刺激时的 Yess／YES 类台词统一为“对”（与 tor_office 一致，共 4 处）；变量后的半角 `!!!` 统一为全角（bar01、tor04 各 1 处，全库 32:2 收敛）；单独省略号 `…` 统一为 `……`（prop、mel01、mel03、tin_vault、tor02、tor04 两处、viv05 共 8 处，全库 9163:10 收敛）。台账 8 个文件重算指纹，`extracted_language_review.json` 追加 14 条记录。
- **90 对档双语复审（第二十五批）**：完成 dia01（99）、mel_office（99）共 2 个文件、198 对逐条英中对照复审，4 处修正。`dia01` 把同一英文句在首次／复访两个分支的不同写法统一：don't be timid 两处统一“别害羞”，R-right 统一“好……好的”，I can do this 统一“我能行的”。`mel_office` 无需改动：skin flute 用“肉箫”保住长笛双关，Miss Melody 用“老师”，naughty boy 用“坏小子”，Good boy 为家族收敛后的“真乖”。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 4 条记录。
- **90 对档双语复审（第二十四批）**：完成 mar_pantry（90）、jud_stall（92）共 2 个文件、182 对逐条英中对照复审，无需改动译文。`mar_pantry` 的呻吟、`Gimme another baby`、事后“歇一会儿”双关与原文一致；`jud_stall` 的 Judith 口吃、征求意见语气与食物式感叹都保持原样。复审中发现 characters.md 里 Judith 感叹词示例（妙极了，松饼／老天，蜂蜜面包）与 terminology.md 及实际译文不一致，已统一为“我的松饼呀／我的蜂蜜面包啊”。台账 2 个文件升级为 `bilingual_scene_read`。
- **70–90 对档双语复审（第二十三批）**：完成 jen13（75）、deb_pants（87）共 2 个文件、162 对逐条英中对照复审，1 处修正：`jen13` 中 Jenny 回应“不客气”的 `Uhh, no?` 原译“呃，不然呢？”意思相反，改“呃，不见得吧。”。`deb_pants` 的支配—照顾语气、憋太久与“好孩子”称呼均与原文和家族规则一致，无需改动。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 1 条记录。
- **70 对档双语复审（第二十二批）**：完成 jen20（73）、oli_stall（73）共 2 个文件、146 对逐条英中对照复审，无需改动。`jen20` 的足交场景呻吟、粗口和 Jenny 的支配口吻与原文一致，What the fuck 已与 jen01 统一为“搞什么鬼”；`oli_stall` 的偷窥道歉、临时调情与“big boy”双关、结尾自嘲都保留原味。台账 2 个文件升级为 `bilingual_scene_read`。
- **惊叹独白标点收敛与 mar_cook 复审（第二十一批）**：单独的 `!!!` 在全库有 153 处作全角“！！！”，其余 42 处半角写法分布在 25 个文件，本轮一并统一；规则补进 style_guide.md 的标点条目，涉及文件重算台账指纹，`extracted_language_review.json` 追加 42 条记录（另修正上一批误记的 153 条，只保留真实改动）。同时完成 `mar_cook`（68 对）全文双语复审：cannoli 用“意式奶油甜馅卷”、奶油馅双关、口交拟声与菜单双关均与原文一致，无需内容改动，仅在专项中统一了 Good boy 与独立 !!!，台账升级为 `bilingual_scene_read`。
- **60–70 对档双语复审（第二十批）**：完成 debbie_rug（67）、jen03（67）共 2 个文件、134 对逐条英中对照复审，3 处修正。`debbie_rug` 的 Amethyst encrusted 改“镶满紫水晶”、bitchzilla 改“母夜叉样”（保留怪物贬称）；`jen03` 的 Anon 台词 being a bitch 由“犯贱”改“无理取闹”，与 deb21 已确认的软化口径一致。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 3 条记录。
- **60 对档双语复审（第十九批）**：完成 tor_office（62）、hana（66）共 2 个文件、128 对逐条英中对照复审，4 处修正。`tor_office` 的高潮台词 Yes, yes, YES!!! 由逐字“是的，是的，是的”改“对，对，对”；`hana` 把 Anon 的 Hopefully with a date／another date 两句补出“带着（另一个）约会对象来”的省略含义，并把 Oh, I'm sure 改“那是当然”。hana 的 -san 敬称、traditional tanto、日语台词加括注等既定处理逐条核对无误。台账 2 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 4 条记录。
- **50–60 对档双语复审（第十八批）**：完成 jen22（59）、tony（59）、titomi（61）共 3 个文件、179 对逐条英中对照复审，无需改动。`jen22` 核对了 anon 的 “you'll be sucking a lot more dick” 主客关系（现译方向正确）；`tony` 的 champ／yes sir 等称呼与术语表一致；`titomi` 的 Omaha／关东／内布拉斯加笑点、Ara Ara 与粗鲁待客口吻均与原文相符。台账 3 个文件升级为 `bilingual_scene_read`。
- **`good boy` 家族全库收敛（第十七批）**：把跨文件漂移的 19 处夸奖句统一到 characters.md 的既定口径——`That's my good boy`→“这才乖嘛”、`That's my boy`→“这才对嘛”、`You're such a good boy`→“你真是个好孩子”、短夸奖 `Good boy`→“真乖”，涉及 12 个文件（deb_tv 6 处、deb_mall 3 处，deb_laundry、deb18、deb19、deb23、ano05、deb_island、jen09、deb25、mar_cook、jen_gfe 各 1 处）。`jen_shower` 的“乖狗狗”狗化玩笑、`If you're a good boy`、`He's such a good boy` 作为 source_exceptions 保留原译。新增 5 条 `recurring_terms.json` 审计规则（含例外），规则同步写进 characters.md 与 style_guide.md；台账 12 个文件重算指纹，`extracted_language_review.json` 追加 18 条并按终稿更新 1 条。
- **50 对档双语复审（第十六批）**：完成 jen19（57）、jen02（58）、jen12（58）共 3 个文件、173 对逐条英中对照复审，3 处修正：`jen02` 把 Debbie 的 You're such a good boy. 统一到照顾语境主流“好孩子”，big drama 不再升格成“天塌了”；`jen12` 把引述中的 a real penis 从易读成粗口的“真鸡巴”改为“真正的鸡巴”。`jen19` 的愤怒／和解段落粗口强度与原文一致，无需改动。台账 3 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 3 条记录。
- **50 对档双语复审（第十五批）**：完成 ano16（52）、jen15（55）、maria（56）、deb_pool（56）共 4 个文件、219 对逐条英中对照复审，1 处修正：`maria` 菜单项 `Just saying hi.` 的“只是来打招呼。”按菜单惯例去掉句末句号。`ano16` 确认 capisce?加括注、champ 用“冠军”等既定口径；`jen15` 的玩具名 Bad Monster 与全库“坏怪物”一致；`deb_pool` 分段呻吟与拒绝语气与原文一致，均无需改动。台账 4 个文件升级为 `bilingual_scene_read`，`extracted_language_review.json` 追加 1 条记录。
- **40 对以上文件双语复审（第十四批）**：完成 deb15（42）、debbie_tv（43）、eve（46）、yoo（47）共 4 个文件、178 对逐条英中对照复审，7 处修正。`debbie_tv` 把西语频道名和两名摔角手译名回归原名（Chicas Luchadoras Televisión、Toxic Terror、Cyclone，与全库保留 Pink Cyclone 一致），清掉两处斜体标签内多余空格，并去掉添加的“呼”；`deb15` 与 `jen01` 的 I can't quite make out- 统一为“我看不太清——”，与 deb_shower 同源句一致。eve、yoo 复核后无需改动（Yoo 的英语口音不另造中文方言，保留粗鲁推销口吻）。台账 4 个文件升级为 `bilingual_scene_read`，`jen01` 重算指纹，`extracted_language_review.json` 追加 8 条记录。
- **40 对以上文件双语复审（第十三批）**：完成 40–44 对区间的 library_shelf（40）、cedric（41）、tool_shovel（43）、deb_sleep（44）、ano04（43）共 5 个文件、211 对逐条英中对照复审，5 处修正。`cedric` 把 getting ripped 由泛泛的“身材越来越好”改“练得越来越精壮”，harpy roommate 恢复贬称“泼妇室友”；`deb_sleep` 与 `deb_lobby` 按 characters.md 与文库主流统一 Debbie 的照顾语境称呼：wonderful boy 用“好孩子”，That's my good boy 用“这才乖嘛”，不写成儿子式称呼。library_shelf、tool_shovel、ano04 复核后无需改动；台账 5 个文件升级为 `bilingual_scene_read`，`deb_lobby` 重算指纹，`extracted_language_review.json` 追加 5 条记录。
- **中小型文件双语复审（第十二批）**：`src/plot` 下 20–39 对区间剩余的 8 个文件（tin01、ten_stall、deb09、tina_lounge、konty、tina、mar_kitchen、jen_visit）完成全文双语复审，共 279 对逐条对照英文，3 处内容修正：`konty` 把被整句汉化的西班牙语 `Llena mis partes traviesas con emocion!` 还原为原文加括注（与同文件保留的 `Sexo en la playa` 一致）；`tin01` 的 You know, for changing! 去掉添加的“顾名思义”；`tina` 的 Now that's a woman 改“这才叫女人”。另把同一源字符串 `*Knock* *Knock*` 在全库的 5 种写法统一为“*咚咚* *咚咚*”（11 处，涉及 ano13、ano15、mar_door、tin01、tin_baby、tin_dusk），规则写进 `style_guide.md`；`tina_lounge` 的开发占位 `tina_lounge.becca` 保持占位字串，与上一批 `maria_lounge` 的处理一致。台账 8 个文件升级为 `bilingual_scene_read`，5 个仅拟声统一文件重算指纹，`extracted_language_review.json` 追加 14 条记录。
- **中小型文件双语复审（第十一批）**：`src/plot` 下 15 个 20–31 对的文件（judith、library_study、kevin、melody、bridget、lily、sam_stall、tammy_bed2_scope、vee、viv、mar02、pie_stall、ari_stall、jen01、jen11）从中文本地连读升级为全文双语复审，共 398 对逐条对照英文，8 处修正。重点：`mar02` 的 knock women off their rockers 原译把“断了她会把人逼疯”译反成“迷得神魂颠倒”；`bridget`、`sam_stall` 去掉两处擅自补出的内容，恢复 piece of meat 的肉体贬损；`pie_stall` 的 close one 与 `jen01` 的惊呼句改按原意表达。同时按全库主流写法把 16 处 `( !!! )`／`（!!!）` 统一为“（！！！）”，并修正三处中文句内的半角括号，规则写进 `style_guide.md`。台账 method 升级为 `bilingual_scene_read`，涉及标点统一的 13 个文件重新登记指纹，`extracted_language_review.json` 追加 26 条修改记录。
...
- **Maria家庭后续**：完成3文件590条中文连读，mar_baby修改5处普通对白：三种出生分支夸赞、来店计划和故事否定接句。累计7095条。9个当前修改RPY结构检查、956条句式回归、间距及覆盖指纹通过。README未改，无RPA构建、场景游玩或提交；全文目标未完成。
- **Tony育儿与Maria日常**：完成4文件373条中文连读，mar_cook普通任务对白润色2处。累计7468条。10个当前修改RPY结构检查、958条句式回归、混排间距和覆盖指纹通过。共享停止菜单与旧术语候选仍待相应语境审校；README未改，无RPA构建、游玩或提交，全文目标未完成。
- **Maria房间场景**：完成3文件764条中文连读，修改4处普通对白：被逗回应、醒着询问、香味夸赞和球赛方向描述。累计8232条。12个当前修改RPY结构检查、962条句式回归、间距与覆盖指纹通过。README未改，无RPA构建、游玩或提交；全文任务未完成。
- **夜间分支衔接复核**：连读mar_dark全文369条，接读ano15收尾，修正ano15_pantry_reuse_06a807eb错误补译。累计8601/37947条。12个当前修改RPY结构校验通过；963条句式回归零不匹配；覆盖记录零过期；中英间距零不匹配。未改README，未构建RPA、游玩或提交。全量审校继续进行。
- **Tina初次与重复入口**：完成5文件266条中文连读及4处普通对白调整。13个当前修改RPY结构检查通过；967条句式回归零不匹配；覆盖零过期、中英间距零不匹配。累计8867条，全文审校未完成。README未改，无构建、游玩或Git提交。
- **Tina约会与金库**：完成371条中文连读，修正金库4处普通对白。14个当前修改RPY结构检查通过，971条句式回归零不匹配，覆盖零过期、中英间距零不匹配。累计9238条；全量审校未完成。README未改，无RPA构建、游玩或提交。
- **Tina孕产对白**：完成322条中文连读，调整14处普通对白，更新规则和本地路径记录。15个当前修改RPY结构检查通过；985条句式回归零不匹配；覆盖零过期、中英间距零不匹配。累计9560条，全文审校仍未完成。README未改，无构建、游玩或提交。
- **银行与车行本地对白**：完成6文件78条中文连读，修正1处短反问衔接。16个当前修改RPY结构校验通过；986条句式回归零不匹配；覆盖零过期、中英间距零不匹配。全文审校未完成。README未改，无构建、游玩或提交。
- **Erik入口与区域观察**：完成7文件78条中文连读，回查疑点后未修改译文；新增逐项覆盖记录。覆盖校验9716条、零过期、无漏列文件。本轮仅更新审校文档和记录，不重复运行无文本变更的结构与句式检查。README未改，无构建、游玩或提交；全文审校未完成。
- **Viv借书任务**：完成278条中文连读，修改8处普通对白。17个当前修改RPY结构校验通过；994条句式回归零不匹配；覆盖零过期、中英间距零不匹配。累计9994条，全文审校仍未完成。README未改，无构建、游玩或提交。
- **Viv前置辅导**：完成viv01的221条中文连读，疑点回查后保留现译。覆盖校验10215条、零过期、无漏列文件。本轮无RPY修改，不重复结构和句式回归。README未改，无构建、游玩或提交；全文审校仍未完成。
- **Viv诗歌及辅导**：完成358条中文连读，修改2处语义与连贯性问题。18个当前修改RPY结构检查通过；996条句式回归零不匹配；覆盖零过期、中英间距零不匹配。README未改，无构建、游玩或提交；全文审校仍未完成。
- **Viv考试前编舞任务**：完成630条中文连读，修改3处普通表达。19个当前修改RPY结构校验通过；999条句式回归零不匹配；覆盖零过期、中英间距零不匹配。README未改，无构建、游玩或提交；全文审校未完成。
- **Viv考试与菜单**：完成4文件187条连读，本轮未改RPY；新增共享菜单语境与法语问答铺垫两个待处理项。覆盖校验11390条、零过期、无漏列文件。README未改，无构建、游玩或提交。全文审校未完成，也未宣称已读区域无问题。
- **共享否定菜单修复**：完成Not really共享菜单四个源使用点的语境复核并修正一条译文。20个当前修改RPY结构检查通过，1000条句式回归零不匹配；覆盖11390条、零过期，中英间距零不匹配。本轮不虚增整场阅读数。README未改，无构建、游玩或提交；全量任务未完成。
- **图书馆本地对白**：完成4文件74条中文连读，未改译文；覆盖11464条、零过期、无漏列文件。只更新对应说明与审校记录，不重复无文本变更的结构测试。README未改，无构建、游玩或提交；全量任务未完成。
- **Judith场景连读**：完成251条中文连读，调整1处甜点惊叹。21个当前修改RPY结构校验通过；1001条句式回归零不匹配；覆盖零过期、中英间距零不匹配。README未改，无构建、游玩或提交；全量审校未完成。
- **学校角色入口**：完成58条中文连读，修正1处菜单主题。22个当前修改RPY结构校验通过；1002条句式回归零不匹配；覆盖零过期、中英间距零不匹配。README未改，无构建、游玩或提交；全文审校未完成。
- **同学入口连读**：完成42条中文连读，未改RPY。覆盖校验11815条、零过期、无漏列文件，未重复无文本变更的结构测试。README未改，无构建、游玩或提交；全量审校未完成。
- **校园区域连读**：完成7文件136条中文连读，保留现译。覆盖校验11951条、零过期、无漏列文件；无RPY修改，不重复结构回归。README未改，无构建、游玩或提交；全文审校未完成。
...
- **Tori主线收尾与重复入口**：Tori收尾及重复入口、血清界面新增238条中文连读，3处修改；累计13899/37947条。全量审校仍未完成。
...
- **演出前夜行动**：mel05新增277条，5处修改；累计15389/37947条，全量审校未完成。
- **才艺演出与收尾**：mel06新增336条，9处修改；累计15725/37947条，全量审校未完成。
- **Melody日常与重复入口**：Melody日常与重复入口新增124条，无RPY修改；累计15849/37947条，全量审校未完成。
- **学校任务道具补读**：道具与校长入口新增22条，无RPY修改；累计15871/37947条，全量审校未完成。
- **Diane开工任务**：Diane开工与取铲子新增142条，1处修改；累计16013/37947条，全量审校未完成。
- **Diane搬运任务**：dia02新增139条，5处修改；累计16152/37947条，全量审校未完成。
- **Diane日常及园艺界面**：Diane日常及园艺界面新增26条，1处修改；累计16178/37947条，全量审校未完成。
- **健身入口与提示**：健身角色与界面新增45条，2处修改；累计16223/37947条，全量审校未完成。
- **教练与餐厅接待**：公共地点入口新增153条，3处修改；累计16376/37947条，全量审校未完成。
- **公共地点接待补读**：公共地点接待和区域提示新增33条，无RPY修改；累计16409/37947条，全量审校未完成。
- **Josie车行开场**：Josie开场新增393条，1处修改；累计16802/37947条，全量审校未完成。
- **购车交易分支**：购车交易与Josie日常新增158条，2处修改；累计16960/37947条，全量审校未完成。
- **披萨店日常补读**：披萨店日常新增13条，无RPY修改；累计16973/37947条，全量审校未完成。
- **Debbie开场连续审校**：Debbie开场三段剧情新增374条，无RPY修改；累计17347/37947条，全量审校未完成。
- **Debbie家务剧情连续审校**：修水管与帮忙家务新增381条，deb05修改4处；累计17728/37947条，全量审校未完成。
- **Debbie按摩后续审校**：deb06新增126条，修改5处；累计17854/37947条，全量审校未完成。
- **电影与购物连续审校**：电影与购物剧情新增522条，修改2处；累计18376/37947条，全量审校未完成。
- **重复购物分支审校**：重复购物及车库路径新增559条，修改7处；累计18935/37947条，全量审校未完成。
- **梦境与醒后对白审校**：梦境与卧室争执新增209条，无RPY修改；累计19144/37947条，全量审校未完成。
- **接吻教学与修车审校**：接吻教学及修车任务新增624条，修改2处；累计19768/37947条，全量审校未完成。
- **卧室与浴室后续审校**：卧室及浴室后续新增240条，修改1处；累计20008/37947条，全量审校未完成。
- **电视与夜访连续审校**：电视与夜访剧情新增347条，无RPY修改；累计20355/37947条，全量审校未完成。
- **入侵与陪睡连续审校**：入侵及失眠陪伴新增491条，修改2处；累计20846/37947条，全量审校未完成。
- **密谈与烘焙节目审校**：厨房密谈与烘焙节目剧情新增459条，无RPY修改；累计21305/37947条，全量审校未完成。
- **和解购物连续审校**：夜访与和解购物新增788条，修改1处；累计22093/37947条，全量审校未完成。
- **泳池与浴袍分支审校**：泳池与寻找浴袍新增342条，无RPY修改；累计22435/37947条，全量审校未完成。
- **夜间探索与回忆审校**：夜间探索与回忆新增428条，修改2处；累计22863/37947条，全量审校未完成。
- **正式约会连续审校**：deb26本地全分支中文连读新增858条，修改4处；累计23721/37947条，整体审校仍未完成。
- **泳池场景连续审校**：泳池首次与重复场景新增189条中文连读覆盖，修正1处短答；累计23910/37947条，全量审校仍未完成。
- **家中入口连续审校**：家中角色入口与环境提示新增245条中文连读覆盖，修改3处；累计24155/37947条，整体未完成。
- **家中物件连续审校**：家中物件提示新增118条中文连读覆盖，修改4处；累计24273/37947条，整体未完成。
- **电视事件连续审校**：电视Debbie事件新增765条中文连读覆盖，修改3处；累计25038/37947条，整体未完成。
- **Jenny电视连续审校**：Jenny电视事件与共用结尾新增218条中文连读覆盖，无译文修改；累计25256/37947条，整体未完成。
- **Jenny主线开端审校**：Jenny主线开端新增219条中文连读覆盖，无译文修改；累计25475/37947条，整体未完成。
- **Jenny照片连续审校**：拍照及回看场景新增241条中文连读覆盖，修改1处；累计25716/37947条，整体未完成。
- **Jenny交易连续审校**：Jenny交易与购买委托新增492条中文连读覆盖，无译文修改；累计26208/37947条，整体未完成。
- **Jenny同行购物审校**：录音转写说辞与同行购物新增477条中文连读覆盖，修改3处；累计26685/37947条，整体未完成。
- **Jenny电脑线索审校**：日记线索与电脑任务新增89条中文连读覆盖，修改2处；累计26774/37947条，整体未完成。
- **Jenny委托回报审校**：送礼与Cedric委托新增260条中文连读覆盖，修改5处；累计27034/37947条，整体未完成。
- **Jenny邀约连续审校**：直播邀约新增331条中文连读覆盖，修改1处；累计27365/37947条，整体未完成。
- **面具任务连续审校**：面具任务新增261条中文连读覆盖，无译文修改；累计27626/37947条，整体未完成。
- **首次直播连续审校**：首次直播及次晨新增237条中文连读覆盖，无译文修改；累计27863/37947条，整体未完成。
- **直播后索款连续审校**：后续直播与索款新增231条中文连读覆盖，无译文修改；累计28094/37947条，整体未完成。
- **望远镜后续审校**：望远镜后续新增206条中文连读覆盖，无译文修改；累计28300/37947条，整体未完成。
- **制服场景连续审校**：制服主题场景新增334条中文连读覆盖，无译文修改；累计28634/37947条，整体未完成。
- **影院场景连续审校**：跟踪者与影院场景完成本地连续阅读，修正2处，新增513条人工覆盖；累计29147/37947条，整体未完成。
- **关系对白连续审校**：关系争执与女友提议场景新增225条人工覆盖，无译文修改；累计29372/37947条。记录原文时间衔接疑点，整体未完成。
- **日常及女友体验连续审校**：日常与女友体验新增529条人工覆盖，修正3条；累计29901/37947条。整体未完成。
- **直播与前戏连续审校**：直播与前戏分支新增291条人工覆盖，无译文修改；累计30192/37947条，整体未完成。
- **泳池餐桌连续审校**：泳池与餐桌场景新增314条人工覆盖，无译文修改；累计30506/37947条，整体未完成。
- **夜间淋浴连续审校**：夜间与淋浴场景新增504条人工覆盖，无译文修改；累计31010/37947条，整体未完成。
- **电脑视频连续审校**：电脑提示与三段视频新增150条人工覆盖，无译文修改；累计31160/37947条，整体未完成。
- **家务场景连续审校**：吸尘与洗衣场景新增439条人工覆盖，无译文修改；累计31599/37947条，整体未完成。
- **厨房洗手台连续审校**：厨房和洗手台新增397条人工覆盖，修正2条；累计31996/37947条，整体未完成。
- **洗衣房入口与卧室连续审校**：洗衣房入口及卧室分支新增100条人工覆盖，修正2条；累计32096/37947条，整体未完成。
- **Debbie夜间来访连续审校**：夜间来访新增296条人工覆盖，修正1条；累计32392/37947条，整体未完成。
- **留宿岛台连续审校**：留宿与厨房岛台新增198条人工覆盖，无译文修改；累计32590/37947条，整体未完成。
- **Debbie淋浴连续审校**：Debbie淋浴新增527条人工覆盖，修正3条；累计33117/37947条，整体未完成。
- **林间及影院连续审校**：林间初遇及影院柜台新增19条人工覆盖，修正1条；累计33136/37947条，整体未完成。
- **设备提示与Roxxy连续审校**：设备提示和Roxxy日常新增69条人工覆盖，无译文修改；累计33205/37947条，整体未完成。
- **物品望远镜连续审校**：零散互动与望远镜场景新增53条人工覆盖，无译文修改；累计33258/37947条，整体未完成。
- **隔间对白连续审校**：隔间对白新增162条人工覆盖，无译文修改；累计33420/37947条，整体未完成。
- **隔间第二批连续审校**：隔间对白新增72条人工覆盖，修正1条；累计33492/37947条，整体未完成。
- **Jenny怀孕第一段连续审校**：怀孕场景第一段新增89条人工覆盖，无译文修改；累计33581/37947条。jen_baby从源259继续，整体未完成。
- **Jenny怀孕第二段连续审校**：怀孕第二段新增116条人工覆盖，修正1条；累计33697/37947条。下一段从623继续，整体未完成。
- **Jenny怀孕第三段连续审校**：怀孕第三段新增157条人工覆盖，无译文修改；累计33854/37947条。下一段从945继续，整体未完成。
- **Jenny怀孕第四段连续审校**：怀孕第四段新增139条人工覆盖，修正2条；累计33993/37947条。下一段从1314继续，整体未完成。
- **Jenny怀孕第五段连续审校**：怀孕第五段新增219条人工覆盖，无译文修改；累计34212/37947条。下一段从1781继续，整体未完成。
- **Jenny怀孕末段连续审校**：Jenny怀孕末段新增209条人工覆盖，修正6条；累计34421/37947条。整体审校仍未完成。
- **Debbie怀孕第一段连续审校**：Debbie怀孕第一段新增317条人工覆盖，修正1条；累计34738/37947条。下一段从847继续，整体未完成。
- **Debbie怀孕第二段连续审校**：Debbie怀孕第二段新增175条人工覆盖，修正3条；累计34913/37947条。整体未完成。
- **Debbie怀孕末段连续审校**：Debbie怀孕末段新增281条人工覆盖，修正1条；累计35194/37947条。剩余界面、动态文本与外部衔接继续审校，整体未完成。
- **电脑界面及邮件连续审校**：电脑页面及邮件新增77条覆盖，修改11处；累计35271/37947条，整体未完成。
- **手机与背包界面审校**：手机、背包与HUD新增27条人工覆盖，修正2处。累计35298/37947条，整体未完成。
- **任务提示首段审校**：任务提示首段修正10条，累计覆盖35364/37947条，整体未完成。
- **任务提示第二段审校**：任务提示第二段修正6条，累计35388/37947条，整体未完成。
- **任务提示第三段审校**：任务提示第三段修正5条，累计35409/37947条，整体未完成。
- **任务提示第四段审校**：任务提示第四段修正5条，累计35443/37947条，整体未完成。
- **任务提示第五段审校**：任务提示第五段修正3条，累计35472/37947条，整体未完成。
- **Pink频道账号修改复核**：jen_tv第31行与jenny_laptop第103行仅将Pink Channel账号改为Pink频道账号；已结合前后文和差异复核，保留修改并刷新两处指纹，不增加人工覆盖。
- **任务提示第六段审校**：任务提示第六段修正4条，累计35483/37947条，整体未完成。
- **任务提示第七段审校**：任务提示第七段修正4条，累计35503/37947条，整体未完成。
- **任务提示第八段审校**：任务提示第八段修正4条，累计35525/37947条，整体未完成。
- **任务提示第九段审校**：任务提示第九段修正5条，累计35550/37947条，整体未完成。
- **任务提示第十段审校**：任务提示第十段修正4条，累计35567/37947条，整体未完成。
- **任务提示第十一段审校**：任务提示第十一段修正5条（含回修611），累计35583/37947条，整体未完成。
- **等待日期漏译补齐**：lang.wait五种返回值原无独立译文，已在bytecode_strings补齐今天、明天、后天、几天后、一周左右后。新增5条独立人工覆盖，总量变为37952条；该文件原627条仍未计覆盖。与step的日期前置表达相接，具体时段映射仍待后续核对。
- **jos_trade菜单修改复核**：复核第823、831行的“算了”“不买了”菜单标点调整及源260、365拒绝购买分支，保留修改并刷新既有覆盖。未增加新覆盖；车型中英相邻无空格已与现记录一致。
- **任务提示第十二段审校**：任务提示第十二段修正6条，累计35614/37952条，整体未完成。
- **任务提示第十三段审校**：任务提示第十三段修正2条，累计35627/37952条，整体未完成。
- **任务提示第十四段审校**：任务提示第十四段修正6条，累计35650/37952条，整体未完成。
...
- **任务提示第二十段审校**：任务提示第二十段修正3条，累计35749/37951条，整体未完成。
- **任务提示第二十一段审校**：任务提示第二十一段修正3条，累计35764/37951条，整体未完成。
...
- **多任务链集中审校**：本批31条修正，新增130条人工阅读覆盖，累计36709/37953。step仍为部分完成，1509起待续；既有语义及外部依赖疑点未清除。
- **剩余任务链批次**：剩余130条任务提示及一条补译已连读登记；修正22条并新增Consum-r精确匹配键。人工覆盖36840/37954。step本地文本已读完，但早期记录的疑点及外部任务条件仍待收尾。
- **道具分组审校**：道具批次本地连读170条并修正16条，人工累计37010/37954。prop源251起待续，典故与任务引用尚未全部结案。
- **道具分组续审**：道具第二批修正13条，阅读覆盖至源372；人工累计37090/37954。
- **道具分组第三批**：道具第三批新增80条人工阅读、修正12条，累计37170/37954。源504起待续；Rump庄园名称差异及药品等后续需统一复核。
- **道具合组收尾**：剩余125条道具说明集体阅读，修正15条，并将Sink恢复到源34位置。累计37295/37954为人工阅读覆盖，不等同于跨事件复核完成。
- **共用界面碎片合组**：共用界面与辅助入口合组：新增33条人工阅读，修正7条。仍保留跨文件共享键、重播入口和字节码日记拼接待审；未执行场景游玩。
- **字节码界面合组**：字节码末段T983起合组阅读，修正两处经调用确认的界面误译。阅读记录不等同于实际日记拼接或短信前后文完成，继续保留外部复核状态。
- **日记事件分页核对**：新增tools/read_diary_pages.py，仅解析字节码注册，不执行游戏代码，不自动授予人工覆盖。已生成事件分页并连读开篇两页。jen15_bed2第7行仅角色变量；jen22_post第9行仅标点颜文字，不是漏译。全日记分页提取成功不代表全日记已读。
- **日记八页合组**：日记下一组合读8页，新增70条阅读覆盖，未修改译文。当前36页中已有10页按事件内顺序连读。页读取记录增加diary_pages事件键、事件内页序与译文行号；事件解锁时序仍待复核。
- **日记旁注与接续合组**：日记再合读8页，新增67条人工覆盖；累计18/36页按事件内行序阅读。未改写译文，完整标签已用于疑点复核，事件解锁及屏幕排版仍未宣称完成。
- **日记剩余十八页合组**：剩余18页日记合组连读完成，新增137条人工阅读记录；36/36页均有事件键与页内行号记录。仅为页内文字覆盖，尚未完成事件解锁时序和跨事件衔接审核。未修改译文。
- **本地文字阅读收尾**：剩余196条字节码文本完成中文初读，修正2条。全库37954条现均有本地文字阅读记录；这并不满足实际场景和分支全审完的目标。下一阶段必须按现有external_event_followup与dependencies逐项核实，不能仅据100%本地覆盖宣告完成。
- **日记写入链与延迟事件**：日记依赖复核推进：已看Book储存/读取、翻页逻辑，以及jen16正常/重试和jen07/08延迟写入函数。manual_review新增event_checks分别记录已确认静态行为与调度层待核，不增加重复人工阅读数量、不取消全局external_event_followup。
- **日记事件返回值复核**：完成日记clear/abort/next调度语义复核，并更新event_checks状态：延迟日记从dispatcher_review_pending缩小为reattachment_review_pending。jen16补核错过入口无日记写入，仍保留入口匹配与互斥待核。未增加文本覆盖数、未修改译文。
- **提取文本按用途拆分（当前维护规则）**：完成提取文本解耦：原聚合文件631条拆为extracted目录下9个独立rpy。已迁移批准译法规则和人工覆盖记录，日记事件页的translation_lines对应新文件行号；原有未完成的事件依赖保持待核。新增extracted_routes.json记录后续增量的源码归属。此次未修改README、未构建RPA、未游玩场景。
- **短信发送者与主语复核**：短信37个注册事件已核对发送者并重读；修正2条指代错误，同步规则与审校行号。事件发送条件仍标trigger_followup，不将注册信息核对等同于时序全部完成。
- **卡拉OK邀请与短信合组**：完成卡拉OK邀请对白与两条Erik回复的调用顺序复核，修正4条口语措辞；保留排期生成与发件历史插入待核。
- **手机消息存储与排期复核**：短信后续：两条Erik回复的调用、储存、显示顺序及rec排期已静态核实；对应message_events关闭这两项trigger_followup。主角发件文本另建insertion_unconfirmed，不把直接引用扫描无结果等同于未使用。其他短信和跨事件任务仍待核。未修改译文。
- **拆分后的短信加载兼容修复**：补修拆分遗漏：sms_fix.rpy原仍读取已删除bytecode_strings.rpy，异常会清空映射。已改为扫描extracted目录，保留原表达式过滤和回退逻辑。隔离测试验证37条短信、307条映射以及新增分类文件；此前仅结构/覆盖校验未覆盖此运行时依赖，现补上回归检查。README未改、未构建RPA、未进行场景游玩。
- **提取文件按出现顺序整理**：按用户要求完成9个提取文件的源码顺序整理，未改动任何old/new内容；日记恢复事件、页、行的连读次序，短信保持注册顺序。未构建RPA、未更改README、未进行游玩验证。
...
- **清理7944的Lint提示（当前结果）**：- 按用户要求撤回README的全部本轮修改，README相对HEAD无差异；不进行场景游玩验证。
...
- **7944完整检查与编译包验证**：- 本轮起点为用户已提交的`b465e25`；以用户指定的`21.0.0-wip.7944`游戏本体为最终依据，在独立目录验证，不安装到用户游戏或改动其存档。
...
- **发包后全仓静态检查与部分语义修订**：保留既有手工修改，对323个RPY文件、38,110对文本进行静态扫描。本轮修订25个文件、123处文本，涵盖人物资料、称谓、指代与中文表达。结构、登记术语和混排空格检查通过；全仓逐句审校及游戏内验证尚未完成。规则、人物信息和待查项已分别归入风格指南、角色档案、术语表与重复表达记录。
